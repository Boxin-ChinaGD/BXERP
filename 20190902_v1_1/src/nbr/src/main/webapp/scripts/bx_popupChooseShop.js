var popupShopData = [];		//商品弹窗所选商品的临时储存位置
var tempShopArray = new Array();		//临时数组
//var needCalculatedAmount = false;		//目前入库页面有使用到
//商品弹窗所选商品监听管理
function popupShopDataManage(status, shopID, shopName){
	switch (status){
		case "add":
			var commodityNotExist = true;
			for(var i=0; i<popupShopData.length; i++){
				if(popupShopData[i].shopID == shopID){
...
//					}
			...
			if(commodityNotExist){		//没有缓存此商品
				var popupCommodity = {};
				popupCommodity["shopID"] = shopID;
				popupCommodity["shopName"] = shopName;
...
//				popupCommodity["specification"] = specification;
				popupShopData.push(popupCommodity);
			}
		break;
		case "delete":
			for(var i=0; i<popupShopData.length; i++){
				if(popupShopData[i].shopID == shopID){
					popupShopData.splice(i, 1);
				}
			}
		break;
	}
	$(".shopFootArea strong").text(popupShopData.length);
	console.log(popupShopData);
}
////商品弹窗商品数量监听管理
...
function notRecordActionsShop(beCopyData, needCopyData){
