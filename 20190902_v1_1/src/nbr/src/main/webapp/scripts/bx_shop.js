layui.use([ 'element', 'form', 'table', 'laypage', "upload", 'laydate' ], function() {
	...
	const updateShop_url = "company/updateShopEx.bx";		//创建一个门店
	const deleteShop_url = "company/deleteShopEx.bx";		//冻结一个门店
//	const createShop_Url = "shop/createEx.bx";		//创建一个门店
...
//	const shopToCheckUniqueField_url = "shop/retrieveNToCheckUniqueFieldEx.bx";		//门店唯一值检测
	...
	var bDesertShopLastOperation = true;		//用于提示是否放弃之前的门店操作，true表示放弃，false表示不放弃。比如，用户点击了修改门店，然后点击某个门店查看详情，这时会弹出提示
	...
	function shopR1 (dbName, shopID) {
		...
		$.ajax({
			...
			success: function succFunction(data){
				...
				layer.close(indexLoading);
				if(data.ERROR == "EC_NoError"){
					var listShopDistrict = data.shopDistrictList;
					...
					form.val("shopInformation", {
						...
						"key": shopInfo.key,
						"districtName":shopInfo.districtName
					});
					// 给门店区域select赋值
//					var listShopDistrict = shopInfo.listShopDistrict;
//					console.log("listShopDistrict" + JSON.stringify(listShopDistrict));
					console.log("shopInfo.districtName" + shopInfo.districtName);
					$("#districtID").empty(); // 清空select
					for (var i = 0; i < listShopDistrict.length; i++) {	
						if(listShopDistrict[i].name == shopInfo.districtName) {
							console.log("listShopDistrict[i].name" + listShopDistrict[i].name);
							$("#districtID").append('<option selected="selected" value="' + listShopDistrict[i].ID + '">' + listShopDistrict[i].name + '</option>');
						}
						else {
							$("#districtID").append('<option value="' + listShopDistrict[i].ID + '">' + listShopDistrict[i].name + '</option>');
						}
					}
					form.render('select', 'shopInformation'); // 实时刷新
//					console.log("shopInfo.districtName" + shopInfo.districtName);
//					$("#districtID").find("option[text='" + shopInfo.districtName +"']").attr("selected",true);
					//渲染pos机信息
	...
	//取消修改按钮
	$(".cancelUpdateShopInfo").click(function(){
		if (bDesertShopLastOperation) {
			shop_reloadTable.pageIndex = 1;
			shopListRN(shop_reloadTable);
		} else {
			layer.confirm('确定要放弃之前的操作吗？', { icon: 3, title: '提示' }, function (index, layero) {		//点击确定
...
//		return;
		...
		switch (text){
			...
			case "修改":
				...
				$(".message .layui-form-select").removeClass("layui-select-disabled")
				$(".message .layui-select-title input").removeClass("layui-disabled")
				form.render('select', 'shopInformation');
				$(".cancelUpdateShopInfo").show();
				bDesertShopLastOperation = false;
			break;
		}
	});
...
//	}
	...
	form.on('submit(shopsubmit)', function(data) {
		layer.confirm('确定修改门店"' + data.field.name + '"吗？', { icon: 3, title: '提示' }, function (index, layero) {		//点击确定
			var indexLoading = layer.load(1);
...
//					"key":data.field.key,
	...
	//取消修改按钮
	$(".toDeleteShop").click(function(){
		if (bDesertShopLastOperation && bDesertShopLastOperation) {
			layer.confirm('确定要冻结该门店吗？', { icon: 3, title: '提示' }, function (index, layero) {		//点击确定
//				layer.close(index);
				$.ajax({
					url : deleteShop_url,
					type : method_post,
					...
					dataType : "json",
					data : {
						"ID":$("#idForDelete_shopID").val(),
						"companyID":$("#idForDelete_companyID").val(),
					},
					success : function succFunction(data) {
						console.log(data);
						if(data.ERROR == "EC_NoError"){
