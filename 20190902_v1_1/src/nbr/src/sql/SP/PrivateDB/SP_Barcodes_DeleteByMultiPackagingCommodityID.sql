-- 删除多包装商品条形码 同时更新条形码的同存
DROP PROCEDURE IF EXISTS `SP_Barcodes_DeleteByMultiPackagingCommodityID`;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Barcodes_DeleteByMultiPackagingCommodityID`(
	OUT iErrorCode INT,
	OUT sErrorMsg VARCHAR(64),
	IN iCommodityID INT, -- 多包装ID。网页上只显示单品的ID给用户删除
	IN iStaffID INT
)
BEGIN
	...
	START TRANSACTION;
		...
		-- 1, 删除X对应的同存
		-- 判断是否是多包装 否则返回错误码7
		IF EXISTS (SELECT 1 FROM t_commodity WHERE F_ID = iCommodityID AND F_TYPE = 2) THEN
			...
			) AND F_ID NOT IN (SELECT F_BarcodeID FROM t_retailtradecommodity rc);-- t_retailtradecommodity有外键依赖,多包装商品不需要考虑采购,入库,退货,盘点
			--  采购、入库、盘点、促销、采购退货的都是普通商品，所以多包装商品不需要检查这些依赖	
			SET iErrorCode := 0;
			SET sErrorMsg := '';
		ELSE 
		    SET iErrorCode := 7;
		    SET sErrorMsg := '不是多包装商品,删除多包装商品的条形码失败';
		END IF ;
