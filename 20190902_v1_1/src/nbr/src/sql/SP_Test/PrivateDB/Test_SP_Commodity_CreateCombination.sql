SELECT '++++++++++++++++++ Test_SP_Commodity_CreateCombination.sql ++++++++++++++++++++';
...
CALL SP_Commodity_CreateCombination(@iErrorCode, @sErrorMsg, @iStatus, @sName, @sShortName, @sSpecification, @iPackageUnitID, @sPurchasingUnit, @sBrandID, @sCategoryID, 
	@sMnemonicCode, @sPricingType, @sPriceVIP, @sPriceWholesale, @sCanChangePrice, @sPicture,@sReturnDays, @sTag, 
...
SELECT '-------------------- Case8:创建一个商品的名称同已删除的商品的名称相同,创建成功 -------------------------' AS 'Case8';
...
AND F_OldValue = ''
AND F_NewValue = @sBarcode;
SELECT IF(found_rows() = 1, '测试成功', '测试失败') AS 'Case8 Testing Result';
-- 
SELECT 1 FROM t_commodityhistory WHERE F_StaffID = @iStaffID 
AND F_ID = (SELECT F_ID FROM t_commodityhistory ORDER BY F_ID DESC LIMIT 1,1)
AND F_OldValue = ''
...
SELECT '-------------------- Case9:条形码的长度为1时创建（条形码长度只能是7至64位） -------------------------' AS 'Case9';
...
SELECT @iErrorCode;
SELECT @sErrorMsg;		
SELECT IF(@iErrorCode = 8, '测试成功', '测试失败') AS 'Case9 Testing Result';