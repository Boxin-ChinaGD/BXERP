...
DELETE FROM t_commodityhistory WHERE F_CommodityID IN (SELECT F_ID FROM t_commodity WHERE F_Name = '可比克薯片(番茄)33');
DELETE FROM t_warehousingcommodity WHERE F_WarehousingID = @warehousingID;
DELETE FROM t_barcodes WHERE F_Barcode = '5sdf31Sd8f321Sd832sd';
DELETE FROM t_commodityshopinfo WHERE F_CommodityID = @iCommodityID;
DELETE FROM t_commodity WHERE F_Name = '可比克薯片(番茄)33';
...
SELECT '-------------------- Case6:用不存在的品牌ID进行创建 -------------------------' AS 'Case6';
SET @iErrorCode = 0;
SET @sErrorMsg = '';
SET @iStatus = 0;
SET @sName = '可比克薯片(番茄)333';
...
SELECT '-------------------- Case7:用不存在的类别ID进行创建 -------------------------' AS 'Case7';
SET @iErrorCode = 0;
SET @sErrorMsg = '';
SET @iStatus = 0;
...
SELECT '-------------------- Case8:用不存在的包装单位ID进行创建 -------------------------' AS 'Case8';
SET @iErrorCode = 0;
SET @sErrorMsg = '';
SET @iStatus = 0;
SET @sName = '可比克薯片(番茄)3333';
...
SELECT '-------------------- Case9:创建的商品与预淘汰商品重名 -------------------------' AS 'Case9';
...
SELECT IF(found_rows() = 1 AND @iErrorCode = 1, '测试成功', '测试失败') AS 'CASE9 Testing Result';

SELECT '-------------------- Case10:创建一个商品的名称同已删除的商品的名称相同,创建成功 -------------------------' AS 'Case10';
...
AND F_OldValue = ''
AND F_NewValue = @sBarcode;
SELECT IF(found_rows() = 1, '测试成功', '测试失败') AS 'CASE10 Testing Result';
-- 
SELECT 1 FROM t_commodityhistory WHERE F_StaffID = @iStaffID 
AND F_ID = (SELECT F_ID FROM t_commodityhistory ORDER BY F_ID DESC LIMIT 1,1)
AND F_OldValue = ''
...
SELECT '-------------------- Case11:条形码的长度为1时创建（条形码长度只能是7至64位） -------------------------' AS 'Case11';
...
SELECT @iErrorCode;
SELECT @sErrorMsg;		
SELECT IF(@iErrorCode = 8, '测试成功', '测试失败') AS 'Case11 Testing Result';

SELECT '-------------------- Case12: 创建期初商品后，入库单的单号不为'' -------------------------' AS 'Case12';
SET @iErrorCode = 0;
SET @sErrorMsg = '';
SET @iStatus = 0;
SET @sName = '可比克薯片(番茄)33';
...
-- 
SELECT IF(@iErrorCode = 0 AND @sSN <> '', '测试成功', '测试失败') AS 'Case12 Testing Result';
-- 
SELECT 1 FROM t_barcodes WHERE F_Barcode = '5sdf31Sd8f321Sd832sd';
SELECT IF(found_rows() = 1, '测试成功', '测试失败') AS 'Case12 Testing Result';
-- 
SET @warehousingID = (SELECT F_WarehousingID FROM t_warehousingcommodity WHERE F_ID = LAST_INSERT_ID());
-- 
SET @iCommodityID = 0;
