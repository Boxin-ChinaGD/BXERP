SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case22.sql+++++++++++++++++++++++';


SELECT '----- case22:员工A创建一个零售单卖A商品，对应两个入库单 ,B商品，对应两个入库单 , C商品，对应两个入库单,一张退货单，对商品A、B都是部分退货, [A]-------' AS 'Case22';
-- 收银员
SET @staffID_A = 9;
...
SET @warehousingCommodityA_Price1 = 9; -- 入库单1
SET @warehousingCommodityA_NO1 = 500;
SET @warehousingCommodityA_Price2 = 8; -- 入库单2
SET @warehousingCommodityA_NO2 = 1000;
SET @commodityPriceB = 220;
SET @warehousingCommodityB_Price1 = 7;
SET @warehousingCommodityB_NO1 = 500;
SET @warehousingCommodityB_Price2 = 6;
SET @warehousingCommodityB_NO2 = 1000;
SET @commodityPriceC = 770;
SET @warehousingCommodityC_Price1 = 5;
SET @warehousingCommodityC_NO1 = 500;
SET @warehousingCommodityC_Price2 = 4;
...
SET @rrt1CommotidyNO2_A = @rt1CommotidyNO2_A; -- 入库单
SET @rrt1CommotidyNO2_B = @rt1CommotidyNO2_B; -- 
SET @rrt1TotalAmount_A = @rrt1CommotidyNO2_A * @rt1CommotidyPrice_A -- 
...
SELECT IF(@iErrorCode = 0, '测试成功', '测试失败') AS 'Case22.1 Testing Result';
-- 
...
SELECT IF(@countDefaultData = 1, '测试成功', '测试失败') AS 'Case22.2 Testing Result';
SET @totalAmountA = 0;
...
SELECT IF(@totalAmountA = (@rt1TotalAmount_A - @rrt1TotalAmount_A), '测试成功', '测试失败') AS 'Case22.3 Testing Result';
-- 
...
SELECT IF(@retailtradeNOA = @rtNoByStaffA, '测试成功', '测试失败') AS 'Case22.4 Testing Result';
-- 
-- 进行业绩报表的毛利结果验证
...
SET @returnGrossMargin = @rrt1TotalAmount_A - (@rrt1CommotidyNO2_A * @warehousingCommodityA_Price2) -- 
					 - (@rrt1CommotidyNO2_B * @warehousingCommodityB_Price2); -- 退货毛利
SELECT IF(@grossMargin = @saleGrossMargin - @returnGrossMargin, '测试成功', '测试失败') AS 'Case22.5 Testing Result';
-- 
