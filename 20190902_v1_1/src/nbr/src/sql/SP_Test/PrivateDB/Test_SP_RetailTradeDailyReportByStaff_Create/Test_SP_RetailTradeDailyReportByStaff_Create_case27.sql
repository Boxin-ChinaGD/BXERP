SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case27.sql+++++++++++++++++++++++';


SELECT '----- case27:员工A创建一个零售单卖A商品，对应两个入库单 ,B商品，对应两个入库单 , C商品，对应两个入库单,A隔天创建一张退货单，对商品A、B全部退货, [A、A]-------' AS 'Case27';
-- 收银员
SET @staffID_A = 9;
...
SET @warehousingCommodityA_Price1 = 9.1111; -- 入库单1
SET @warehousingCommodityA_NO1 = 500;
SET @warehousingCommodityA_Price2 = 8.22228; -- 入库单2
SET @warehousingCommodityA_NO2 = 1000;
SET @commodityPriceB = 220;
SET @warehousingCommodityB_Price1 = 7.3333;
SET @warehousingCommodityB_NO1 = 500;
SET @warehousingCommodityB_Price2 = 6.644466;
SET @warehousingCommodityB_NO2 = 1000;
SET @commodityPriceC = 770;
SET @warehousingCommodityC_Price1 = 5.56655;
SET @warehousingCommodityC_NO1 = 500;
SET @warehousingCommodityC_Price2 = 4.47777;
...
SET @rrt1CommotidyNO1_B = @rt1CommotidyNO1Yesterday_B;
...
SET @rrt1TotalAmount_A = (@rrt1CommotidyNO1_A + @rrt1CommotidyNO2_A) * @rt1CommotidyPriceYesterday_A -- 
		+ (@rrt1CommotidyNO1_B + @rrt1CommotidyNO2_B) * @rt1CommotidyPriceYesterday_B; -- 退货总额
...
-- 创建零售退货单商品B
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@returnIDB, @commIDB, '普通商品B', @barcodeIDB, (@rrt1CommotidyNO1_B + @rrt1CommotidyNO2_B), @commodityPriceB, (@rrt1CommotidyNO1_B + @rrt1CommotidyNO2_B), @rt1CommotidyPriceYesterday_B, 200, NULL);
...
SET @returnGrossMargin = (@rrt1CommotidyNO1_A * @warehousingCommodityA_Price1) + (@rrt1CommotidyNO2_A * @warehousingCommodityA_Price2) -- 
					 + (@rrt1CommotidyNO1_B * @warehousingCommodityB_Price1) + (@rrt1CommotidyNO2_B * @warehousingCommodityB_Price2) - @rrt1TotalAmount_A; -- 退货毛利
