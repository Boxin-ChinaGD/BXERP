SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case76.sql+++++++++++++++++++++++';

SELECT '----- case76:  员工A创建一个零售单卖A商品，商品A为未入库商品 ,卖B商品，对应一个入库单，员工B创建退货单，对A全部退货，对商品B全部退货 [AB] -------' AS 'Case76';
-- 
-- 员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
-- 
-- 创建零售单
SET @saleCommANO_A = 230; -- 未入库，直接卖500
SET @aCommotidyPrice = 671.030000;
SET @saleCommBNO_A = 120;	-- 入库500，卖100
SET @bCommotidyPrice = 220.570000;
...
VALUES (@staffIDA, 2, @rggWorkTimeStart, @rggWorkTimeEnd, 1, @rgg1TotalAmount_A, 100.000000, @rgg1TotalAmountCash_A, @rgg1TotalAmountWeChat_A, @rgg1TotalAmountAliPay_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgID_A = last_insert_id();
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDB, 2, @rggWorkTimeStart, @rggWorkTimeEnd, 1, @rgg2TotalAmount_B, 100.000000, @rgg2TotalAmountCash_B, @rgg2TotalAmountWeChat_B, @rgg2TotalAmountAliPay_B, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case76 Result';
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case76 Result';
-- 
