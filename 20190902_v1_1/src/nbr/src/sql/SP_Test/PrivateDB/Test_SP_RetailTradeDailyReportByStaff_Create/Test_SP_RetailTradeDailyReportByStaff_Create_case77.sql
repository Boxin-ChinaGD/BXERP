SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case77.sql+++++++++++++++++++++++';

SELECT '----- case77:  员工A创建一个零售单卖A商品，商品A为未入库商品 ,卖B商品，对应一个入库单，员工B创建退货单，对A全部退货，对商品B全部退货 [A、B] -------' AS 'Case77';
-- 
-- 员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
-- 插入入库单商品表
SET @warehouseBNO = 502; -- 入库数量500
SET @warehouseBPrice = 23.61; -- 入库价10
...
-- 
-- 创建零售单
SET @saleCommANO_A = 403; -- 未入库，直接卖500
SET @aCommotidyPrice = 300.000000;
SET @saleCommBNO_A = 212;	-- 入库500，卖100
SET @bCommotidyPrice = 213.230000;
...
VALUES (@rtCommBID_A, @commIDB, @saleCommBNO_A, @warehouseB);
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case77 Result';
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case77 Result';
-- 
