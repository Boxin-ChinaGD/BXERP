SELECT '++++++++++++++++++ Test_SP_CommoditySyncCache_POSUpload.sql ++++++++++++++++++++';
...
CALL SP_CommoditySyncCache_POSUpload(@iErrorCode, @sErrorMsg, @iSyncData_ID, 1, @cSyncType, @iPOSID);

SELECT @sErrorMsg;
SELECT 1 FROM T_commoditySyncCache WHERE F_SyncData_ID = @iSyncData_ID AND F_SyncType = @cSyncType;
SELECT IF(found_rows() = 1 AND @iErrorCode = 0, '≤‚ ‘≥…π¶', '≤‚ ‘ ß∞‹') AS 'Case 1 Testing Result';
SELECT 1 FROM t_commoditysynccachedispatcher WHERE F_POS_ID = @iPOSID;
SELECT IF(found_rows() = 1 AND @iErrorCode = 0, '≤‚ ‘≥…π¶', '≤‚ ‘ ß∞‹') AS 'Case 1 Testing Result';
...
SELECT 1 FROM t_commoditysynccachedispatcher WHERE F_SyncCacheID = last_insert_id() AND F_POS_ID = @iPOSID;
