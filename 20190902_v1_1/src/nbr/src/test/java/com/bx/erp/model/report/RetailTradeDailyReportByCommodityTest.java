...
public class RetailTradeDailyReportByCommodityTest {
	@BeforeClass
	public void setup() {
	...
	public void checkCreate() {
		...
		RetailTradeDailyReportByCommodity r = new RetailTradeDailyReportByCommodity();
		String error = "";
	...
	public void checkRetrieveN() throws Exception {
		...
		Assert.assertEquals(error, RetailTradeDailyReportByCommodity.FIELD_ERROR_queryKeyword);
		r.setQueryKeyword("这是商品名称");
		// 测试升序
		r.setIsASC(9);
		error = r.checkRetrieveN(BaseBO.INVALID_CASE_ID);
		Assert.assertEquals(error, RetailTradeDailyReportByCommodity.FIELD_ERROR_isASC);
		r.setIsASC(0);
		// 测试排序
		r.setiOrderBy(9);
		error = r.checkRetrieveN(BaseBO.INVALID_CASE_ID);
		Assert.assertEquals(error, RetailTradeDailyReportByCommodity.FIELD_ERROR_iOrderBy);
		r.setiOrderBy(0);
		// 测试int2
		r.setbIgnoreZeroNO(9);
		error = r.checkRetrieveN(BaseBO.INVALID_CASE_ID);
		Assert.assertEquals(error, RetailTradeDailyReportByCommodity.FIELD_ERROR_bIgnoreZeroNO);
		r.setbIgnoreZeroNO(0);
		// 测试int3
		...
		r.setiCategoryID(-2);
		error = r.checkRetrieveN(BaseBO.INVALID_CASE_ID);
		Assert.assertEquals(error, RetailTradeDailyReportByCommodity.FIELD_ERROR_iCategoryID);
		r.setiCategoryID(1);
		// 测试PageIndex
		r.setPageIndex(0);
		error = r.checkRetrieveN(BaseBO.INVALID_CASE_ID);
