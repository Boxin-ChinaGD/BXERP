...
public class PromotionCalculator {
    ...
    public RetailTrade sell(List<Commodity> commList, List<BaseModel> promotionList, RetailTradePromoting retailTradePromoting) {
        ...
        List<RetailTradeCommodity> retailTradeNormalCommodityList = new ArrayList<>();
        // 创建一个临时订单商品表（多包装、组合商品、服务商品不能参与促销）
        List<RetailTradeCommodity> retailtradeOtherCommodityList = new ArrayList<>();

        for (Commodity commodity : commList) {
            RetailTradeCommodity rc = new RetailTradeCommodity();
            rc.setCommodityID(commodity.getID().intValue());
            ...
            rc.setPriceOriginal(commodity.getPriceRetail());
        ...
        if (coa.getMapRetailTradePromotingFlow().size() != 0 || doa.getMapRetailTradePromotingFlow().size() != 0) {
            ...
            if (rtCoaCloned.getAmount() > rtDoaCloned.getAmount()) {
                ...
                if (doa.promotionBest != null) {
                    RetailTradePromotingFlow retailTradePromotingFlows = doa.getMapRetailTradePromotingFlow().get(doa.promotionBest.getID().intValue());
                ...
                if (coa.promotionBest != null) {
                    RetailTradePromotingFlow retailTradePromotingFlows = coa.getMapRetailTradePromotingFlow().get(coa.promotionBest.getID().intValue());
                    retailTradePromotingFlowArrayList.add(retailTradePromotingFlows);
                }
            }
        } else {
        ...
        if (coa.getMapRetailTradePromotingFlow().size() != 0 || doa.getMapRetailTradePromotingFlow().size() != 0) {
            ...
            if (rtCoaCloned.getAmount() > rtDoaCloned.getAmount()) {
                ...
                if (doa.promotionAllBest != null) {
                    RetailTradePromotingFlow retailTradePromotingFlows = doa.getMapRetailTradePromotingFlow().get(doa.promotionAllBest.getID().intValue());
                    retailTradePromotingFlowArrayList.add(retailTradePromotingFlows);
                }
            } else {
                bestRt = rtCoaCloned;
                ...
                if (coa.promotionAllBest != null) {
                    RetailTradePromotingFlow retailTradePromotingFlows = coa.getMapRetailTradePromotingFlow().get(coa.promotionAllBest.getID().intValue());
                    retailTradePromotingFlowArrayList.add(retailTradePromotingFlows);
                }
            }
        } else {
