...
public class Shared {
    ...
    public final static long SQLITE_ID_Infinite = 10000000000l;
    private static CommodityPresenter commodityPresenter = GlobalController.getInstance().getCommodityPresenter();
...
//    }
     ...
     * @throws
     */
    public static boolean login(long posID, PosLoginHttpBO posLoginHttpBO, StaffLoginHttpBO staffLoginHttpBO) throws InterruptedException { //... 重载一个方法 需要传BO
...
//        return true;
//    }
    public static boolean login(long posID, String username, String password, int bx, String companySN, PosLoginHttpBO posLoginHttpBO, StaffLoginHttpBO staffLoginHttpBO, int isLoginFromPos) throws InterruptedException {
        Constants.posID = (int) posID;
...
//        Constants.MyCompanySN = nbr_CompanySN;
//        //1.pos登录
//        BaseActivity.posID = p.getID().intValue();
//        Pos pos = new Pos();
//        pos.setID(p.getID());
...
//    }
    ...
    // ...GlobalController的对应BO和EVENT解除联系后重载
    public static boolean login(Pos p, String companySN, PosLoginHttpBO posLoginHttpBO, StaffLoginHttpBO staffLoginHttpBO) throws InterruptedException {
        Constants.MyCompanySN = companySN;
        //1.pos登录
        Constants.posID = p.getID().intValue();
        Pos pos = new Pos();
        pos.setID(p.getID());
        pos.setResetPasswordInPos(p.getResetPasswordInPos());
        pos.setPasswordInPOS(p.getPasswordInPOS());
...
//        }
     ...
     */
    public static void retrieveNCBaseModel(BaseModel bm, BaseHttpBO baseHttpBO, BaseSQLiteEvent baseSQLiteEvent, BaseSQLiteBO baseSQLiteBO) throws Exception {
        ...
        do {
            ...
            if (baseHttpBO.getHttpEvent().getCount() != null && !"".equals(baseHttpBO.getHttpEvent().getCount())) {
                int count = Integer.valueOf(baseHttpBO.getHttpEvent().getCount());
