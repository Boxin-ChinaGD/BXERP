...
public class Vip extends BaseModel {
    ...
    @Column(name = "F_ConsumeTimes")
    protected int consumeTimes;//总消费次数

    @Column(name = "F_ConsumeAmount")
    ...
    @Column(name = "F_LastConsumeDatetime")
    protected Date lastConsumeDatetime;//上次消费日期时间

    @Column(name = "F_LocalPosSN")
    ...
    //@Generated(hash = 649821198)
//    public Vip(Long ID, //@NotNull String iCID, int startRetailTreadeIDInSQLite, //@NotNull String mobile, //@NotNull String name, //@NotNull String email, //@NotNull String account, //@NotNull String password, int consumeTimes, double consumeAmount, //@NotNull String district,
//            int category, int status, //@NotNull Date birthday, int bonus, //@NotNull Date lastConsumeDatetime, int IDInPOS, //@NotNull String POS_SN, Date syncDatetime, String syncType) {
//        this.ID = ID;
//        this.iCID = iCID;
...
//    }
    ...
    public String toString() {
        return "Vip{" +
                ...
                ", ID=" + ID +
                ", iCID='" + iCID + '\'' +
                ", startRetailTreadeIDInSQLite=" + startRetailTreadeIDInSQLite +
                ", mobile='" + mobile + '\'' +
                ", name='" + name + '\'' +
                ", email='" + email + '\'' +
                ...
                ", birthday=" + birthday +
                ", bonus=" + bonus +
                ", lastConsumeDatetime=" + lastConsumeDatetime +
                ", localPosSN='" + localPosSN + '\'' +
                ", vipList=" + vipList +
                ", isSelect=" + isSelect +
                ...
                ", cardCode='" + cardCode + '\'' +
                ", vipCardSN='" + vipCardSN + '\'' +
                ", bQuerySmallerThanStartID=" + bQuerySmallerThanStartID +
                '}';
    }
...
//                && vip.getIDInPOS() == this.getIDInPOS() && printComparator("IDInPOS")//
...
//                && vip.getAccount().equals(this.getAccount()) && printComparator("account")//
//                && vip.getPassword().equals(this.getPassword()) && printComparator("password")//
...
//                && vip.getStatus() == this.getStatus() && printComparator("status")//
...
//        vip.setIDInPOS(this.getIDInPOS());
//        vip.setPOS_SN(this.getPOS_SN());
        vip.setiCID(this.getICID());
        vip.setName(this.getName());
        vip.setEmail(this.getEmail());
//        vip.setAccount(this.getAccount());
//        vip.setPassword(this.getPassword());
...
//        vip.setStatus(this.getStatus());
    ...
    public BaseModel doParse1(JSONObject object) {
        ...
        try {
            ID = object.getInteger(field.getFIELD_NAME_ID());
            mobile = object.getString(field.getFIELD_NAME_mobile());
//            IDInPOS = object.getInteger(field.getFIELD_NAME_IDInPOS());
...
//            account = object.getString(field.getFIELD_NAME_account());
//            password = object.getString(field.getFIELD_NAME_password());
            consumeTimes = object.getInteger(field.getFIELD_NAME_consumeTimes());
            ...
            category = object.getInteger(field.getFIELD_NAME_category());
//            status = object.getInteger(field.getFIELD_NAME_status());
            bonus = object.getInteger(field.getFIELD_NAME_bonus());
            //
    ...
    public BaseModel parse1(String s) {
        ...
        try {
            JSONObject joVIP = JSONObject.parseObject(s);
...
//        if (printCheckField(field.getFIELD_NAME_IDInPOS(), FIELD_ERROR_IDInPOS, sbError) && IDInPOS != BaseSQLiteBO.INVALID_INT_ID //
//                && IDInPOS != 0 && !FieldFormat.checkID(IDInPOS)) {
//            return sbError.toString();
//        }
//        if (printCheckField(field.getFIELD_NAME_POS_SN(), Pos.FIELD_ERROR_PosSN, sbError) && !StringUtils.isEmpty(POS_SN) && !FieldFormat.checkPosSN(POS_SN)) {
//            return sbError.toString();
//        }
...
//        if (printCheckField(field.getFIELD_NAME_account(), FIELD_ERROR_Account, sbError) && !StringUtils.isEmpty(account) && !FieldFormat.checkAccount(account)) {
//            return sbError.toString();
//        }

//        if (printCheckField(field.getFIELD_NAME_password(), FIELD_ERROR_password, sbError) && !StringUtils.isEmpty(password) && !FieldFormat.checkPassword(password)) {
//            return sbError.toString();
//        }
...
//        if (printCheckField(field.getFIELD_NAME_status(), FIELD_ERROR_status, sbError) && !(status == EnumStatusVip.ESV_Normal.index || status == EnumStatusVip.ESV_LossDeclaration.index || status == EnumStatusVip.ESV_Deleted.index)) {
//            return sbError.toString();
//        }
                ...
                switch (subUseCaseID) {
                    ...
                    case ESUC_Long_String:
                        if (conditions.length == 2) {
                            ...
                            if (printCheckField(field.getFIELD_NAME_POS_SN(), Pos.FIELD_ERROR_PosSN, sbError) && !StringUtils.isEmpty(conditions[1]) && !FieldFormat.checkPosSN(conditions[1])) {
                                return sbError.toString();
                            }
...
//                }
                ...
                if (printCheckField(field.getFIELD_NAME_mobile(), FIELD_ERROR_mobile, sbError) && !StringUtils.isEmpty(mobile) && !FieldFormat.checkMobile(mobile)) {
                    return sbError.toString();
                }
                break;
        }
