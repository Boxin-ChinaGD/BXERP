...
@Component("brandSyncCacheBO")
@Scope("prototype")
public class BrandSyncCacheBO extends BaseBO {
	protected final String SP_BrandSyncCache_RetrieveN = "SP_BrandSyncCache_RetrieveN";
	
	@Resource
	private BrandSyncCacheMapper brandSyncCacheMapper;

	@Override
	protected void setMapper() {
		this.mapper = brandSyncCacheMapper;
	}

	@Override
	protected BaseModel doDelete(int iUseCaseID, BaseModel s) {
		switch (iUseCaseID) {
		case CASE_X_DeleteAllBrandSyncCache:
			Map<String, Object> params1 = s.getDeleteParam(iUseCaseID, s);
			BaseModel bm1 = ((BrandSyncCacheMapper) mapper).deleteAll(params1);
	...
	protected boolean checkRetrieveNPermission(int staffID, int iUseCaseID, BaseModel s) {
		switch (iUseCaseID) {
		default:
			return checkStaffPermission(staffID, SP_BrandSyncCache_RetrieveN);
		}
	}
}
