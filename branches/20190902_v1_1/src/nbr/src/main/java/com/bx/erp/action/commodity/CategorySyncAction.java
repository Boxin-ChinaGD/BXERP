...
@Controller
@RequestMapping("/categorySync")
@Scope("prototype")
public class CategorySyncAction extends BaseSyncAction {
	private Log logger = LogFactory.getLog(CategorySyncAction.class);
	...
	@Resource
	protected com.bx.erp.cache.commodity.CategorySyncCache csc;
	...
	protected EnumCacheType getCacheType() {
		return EnumCacheType.ECT_Category;
	}
	...
	protected EnumErrorCode deleteSlaveDBObjects(BaseModel bmMasterFromDB, ModelMap model, HttpServletRequest req, String dbName) {
		return EnumErrorCode.EC_NoError;// Category没有从表数据
	}
	...
	protected EnumErrorCode createSlaveDBObjects(BaseModel bmMasterFromDB, ModelMap model, HttpServletRequest req, String dbName) {
		return EnumErrorCode.EC_NoError;// Category没有从表
	}

	/*
	 * name=测试 post URL:http://localhost:8080/nbr/categorySync/createEx.bx
	 */
	@RequestMapping(value = "/createEx", produces = "plain/text; charset=UTF-8", method = RequestMethod.POST)
	@ResponseBody
	public String createEx(@ModelAttribute("SpringWeb") Category category, ModelMap model, HttpSession session, HttpServletRequest req) {
		if (!canCallCurrentAction(session, EnumUserScope.STAFF.getIndex())) {
			logger.debug("无权访问本Action");
			return null;
		}
		
		logger.info("创建一个类别的同步缓存,category=" + category);
		...
		try {
			m = handleCreateEx(false, category, model, req, company.getDbName());
		} catch (Exception e) {
	...
	/*
	 * ID=1&name=测试 post URL:http://localhost:8080/nbr/categorySync/updateEx.bx
	 */
	@RequestMapping(value = "/updateEx", produces = "plain/text; charset=UTF-8", method = RequestMethod.POST)
	@ResponseBody
	public String updateEx(@ModelAttribute("SpringWeb") Category category, ModelMap model, HttpSession session, HttpServletRequest req) {
		if (!canCallCurrentAction(session, EnumUserScope.STAFF.getIndex())) {
			logger.debug("无权访问本Action");
			return null;
		}
		
		logger.info("修改一个类别的同步缓存,category=" + category);
		...
		try {
			m = handleUpdateEx(false, BaseBO.INVALID_CASE_ID, category, model, req, company.getDbName());
		} catch (Exception e) {
	...
	/*
	 * get URL:http://localhost:8080/nbr/categorySync/retrieveNEx.bx
	 */
	@RequestMapping(value = "/retrieveNEx", produces = "plain/text;charset=UTF-8", method = RequestMethod.GET)
	@ResponseBody
	public String retrieveNEx(@ModelAttribute("SpringWeb") Category category, ModelMap model, HttpServletRequest req) {
		if (!canCallCurrentAction(req.getSession(), EnumUserScope.STAFF.getIndex())) {
			logger.debug("无权访问本Action");
			return null;
		}
		
		logger.info("查询多个类别的同步缓存,category=" + category);
		...
		Map<String, Object> m = null;
		try {
			m = handleRetrieveNEx(((Pos) req.getSession().getAttribute(EnumSession.SESSION_POS.getName())).getID(), category, model, req, company.getDbName());
		} catch (Exception e) {
	...
	/*
	 * get
	 * URL:http://localhost:8080/nbr/categorySync/feedbackEx.bx?sID=1,2&errorCode=
	 * EC_NoError
	 */
	@RequestMapping(value = "/feedbackEx", produces = "plain/text;charset=UTF-8", method = RequestMethod.GET)
	@ResponseBody
	public String feedbackEx(@ModelAttribute("SpringWeb") Category category, ModelMap model, HttpServletRequest req) {
		if (!canCallCurrentAction(req.getSession(), EnumUserScope.STAFF.getIndex())) {
			logger.debug("无权访问本Action");
			return null;
		}
		
		logger.info("POS机告知服务器已经同步成功,category=" + category);
		...
		try {
			param = handleFeedbackEx(false, category, model, req, company.getDbName());
		} catch (Exception e) {
			e.printStackTrace();
			param = null;
		}
	...
	/*
	 * get URL:http://localhost:8080/nbr/categorySync/deleteEx.bx?ID=1
	 */
	@RequestMapping(value = "/deleteEx", produces = "plain/text; charset=UTF-8", method = RequestMethod.GET)
	@ResponseBody
	public String deleteEx(@ModelAttribute("SpringWeb") Category category, ModelMap model, HttpServletRequest req) {
		if (!canCallCurrentAction(req.getSession(), EnumUserScope.STAFF.getIndex())) {
			logger.debug("无权访问本Action");
			return null;
		}
		
		logger.info("准备删除一个类别,category=" + category);
		...
		try {
			m = handleDeleteEx(false, category, model, req, company.getDbName(), BaseBO.INVALID_CASE_ID);
		} catch (Exception e) {
			e.printStackTrace();
			m = null;
		}
