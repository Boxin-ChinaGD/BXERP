...
@Component("brandSyncCache")
@Scope("prototype")
public class BrandSyncCache extends SyncCache {
//	private Log logger = LogFactory.getLog(BrandSyncCache.class);

	public Hashtable<String, BaseModel> ht;

	@Resource
	private BrandSyncCacheBO brandSyncCacheBO;

	@Resource
	private BrandSyncCacheDispatcherBO brandSyncCacheDispatcherBO;

	public BrandSyncCache() {
		sCacheName = "同步缓存块(品牌)";
	}
	...
	protected BaseModel getNewSyncCacheModel() {
		return (BaseModel) new com.bx.erp.model.commodity.BrandSyncCache();
	}

	@Override
	protected BaseModel getNewSyncCacheDispatcherModel() {
		return (BaseModel) new com.bx.erp.model.commodity.BrandSyncCacheDispatcher();
	}
	...
	protected int getSyncCacheType() {
		return EnumSyncCacheType.ESCT_BrandSyncInfo.getIndex();
	}
	@Override
	...
	protected void doDelete1(BaseModel o) {
		if (o instanceof com.bx.erp.model.commodity.BrandSyncCache) {
			super.doDelete1(o);
		} else if (o instanceof Brand) {
			List<com.bx.erp.model.commodity.BrandSyncCache> lsPointer = new ArrayList<com.bx.erp.model.commodity.BrandSyncCache>();

			for (Iterator<String> it = getCache().keySet().iterator(); it.hasNext();) {
				com.bx.erp.model.commodity.BrandSyncCache rsInfo = (com.bx.erp.model.commodity.BrandSyncCache) getCache().get(it.next());
				if (rsInfo.getSyncData_ID() == o.getID()) {
					lsPointer.add(rsInfo);
				}
			}

			for (com.bx.erp.model.commodity.BrandSyncCache rtsi : lsPointer) {
