DROP PROCEDURE IF EXISTS `SP_Commodity_DeleteMultiPackaging`;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Commodity_DeleteMultiPackaging`(
	OUT iErrorCode INT,
...
BEGIN
	...
	START TRANSACTION;
		...
		-- 判断是否是多包装商品
		IF EXISTS (SELECT 1 FROM t_commodity WHERE F_ID = iID AND F_TYPE <> 2) THEN
		    SET iErrorCode := 7;
		    SET sErrorMsg := '非多包装商品';
		...
		ELSEIF sCheckDependency = '' THEN
			CALL SP_Barcodes_DeleteByMultiPackagingCommodityID(iErrorCode, sErrorMsg, iID, iStaffID); -- 删除多包装商品的条形码,同时更新条形码的普存和同存
