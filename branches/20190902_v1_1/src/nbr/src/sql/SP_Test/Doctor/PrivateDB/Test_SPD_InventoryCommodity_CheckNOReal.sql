...
SELECT '-------------------- Case2:盘点单商品的实盘数量小于0 -------------------------' AS 'Case2';
...
VALUES (@iInventorysheetID, 1, '薯片', 1, 1, 1, -1, 0);
...
SELECT @iErrorCode, @sErrorMsg;
-- 
SELECT IF(@sErrorMsg = CONCAT('盘点单商品', @iInventoryCommodityID ,'实盘数量不正确，它要大于等于0') AND @iErrorCode = 7, '测试成功', '测试失败') AS 'Case2 Testing Result';
-- 
