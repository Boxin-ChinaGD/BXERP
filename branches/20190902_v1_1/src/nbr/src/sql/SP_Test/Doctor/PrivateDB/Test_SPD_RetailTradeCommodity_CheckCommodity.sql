SELECT '++++++++++++++++++ Test_SPD_RetailTradeCommodity_CheckCommodity.sql ++++++++++++++++++++';
...
SET @sErrorMsg = '';
-- 
CALL SPD_RetailTradeCommodity_CheckCommodity(@iErrorCode, @sErrorMsg);
SELECT @iErrorCode;
SELECT @sErrorMsg;
-- 
...
-- 该测试存在外键约束
-- 零售单商品对应的商品不是商品表中的商品
-- SELECT '-------------------- Case2:零售单商品对应的商品不是商品表中的商品 -------------------------' AS 'Case2';
-- 
-- SET @iErrorCode = 0;
-- SET @sErrorMsg = '';
-- 插入一条零售单数据，测试完会删除
...
-- VALUES (@iRetailTradeID, -1,'不存在的商品', 1, 20, 222.6, 0.9, 20, 222.6, 200);
...
-- VALUES (@iRetailTradeCommodityID, 20, 24, -1);
...
-- SELECT @sErrorMsg;
-- 
-- SELECT IF(@sErrorMsg = CONCAT('ID为', @iRetailTradeCommodityID, '的零售单商品的库存必须大于0'); AND @iErrorCode = 7, '测试成功', '测试失败') AS 'Case2 Testing Result';
...
-- 零售单商品的商品数量等于0
SELECT '-------------------- Case3:零售单商品的商品数量等于0 -------------------------' AS 'Case3';
-- 
SET @iErrorCode = 0;
SET @sErrorMsg = '';
-- 插入一条商品数据，测试完会删除
...
3,30,'2018-04-14 01:00:01','20',0,0,'1111111', 0,0);
...
VALUES (@iRetailTradeID, @iCommodityID,'可比克薯片aaa', 1, 0, 222.6, 20, 222.6, 200);
SET @iRetailTradeCommodityID = LAST_INSERT_ID();
-- 插入一条零售单商品来源数据，测试完会删除
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_NO, F_WarehousingID, F_ReducingCommodityID)
...
SELECT @sErrorMsg;
-- 
SELECT IF(@sErrorMsg = CONCAT('ID为', @iRetailTradeCommodityID, '的零售单商品的库存必须大于0') AND @iErrorCode = 7, '测试成功', '测试失败') AS 'Case3 Testing Result';
DELETE FROM t_retailtradecommoditysource WHERE F_ID = @iRetailtradecommoditysourceID;
DELETE FROM t_retailtradecommodity WHERE F_ID = @iRetailTradeCommodityID;
DELETE FROM t_retailtrade WHERE F_ID = @iRetailTradeID;
...
-- 零售单商品的商品数量小于0
SELECT '-------------------- Case4:零售单商品的商品数量小于0 -------------------------' AS 'Case4';
-- 
SET @iErrorCode = 0;
SET @sErrorMsg = '';
-- 插入一条商品数据，测试完会删除
...
-- 插入一条零售单商品数据，测试完会删除
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID,F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer)
VALUES (@iRetailTradeID, @iCommodityID,'可比克薯片aaa', 1, -1, 222.6, 20, 222.6, 200);
SET @iRetailTradeCommodityID = LAST_INSERT_ID();
-- 插入一条零售单商品来源数据，测试完会删除
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_NO, F_WarehousingID, F_ReducingCommodityID)
VALUES (@iRetailTradeCommodityID, 20, 24, @iCommodityID);
...
SELECT @sErrorMsg;
-- 
SELECT IF(@sErrorMsg = CONCAT('ID为', @iRetailTradeCommodityID, '的零售单商品的库存必须大于0') AND @iErrorCode = 7, '测试成功', '测试失败') AS 'Case4 Testing Result';
DELETE FROM t_retailtradecommoditysource WHERE F_ID = @iRetailtradecommoditysourceID;
DELETE FROM t_retailtradecommodity WHERE F_ID = @iRetailTradeCommodityID;
DELETE FROM t_retailtrade WHERE F_ID = @iRetailTradeID;
DELETE FROM t_commodity WHERE F_ID = @iCommodityID;
