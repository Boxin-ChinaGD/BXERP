SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case20.sql+++++++++++++++++++++++';

SELECT '----- case20:员工A创建一个零售单卖A商品，对应两个入库单 ,B商品，对应两个入库单 , C商品，对应两个入库单,一张退货单，对商品A全部退货, [A]-------' AS 'Case20';
-- 
SET @rt1CommotidyNO1 = 500;
SET @rt1CommotidyPrice1 = 300;
SET @rt1CommotidyNO2 = 100;
SET @rt1CommotidyPrice2 = 200;
...
SET @rt1TotalAmount = @rt1CommotidyNO1 * @rt1CommotidyPrice1 + @rt1CommotidyNO2 * @rt1CommotidyPrice2 + @rt1CommotidyNO3 * @rt1CommotidyPrice3;
-- 
SET @rrt1CommotidyNO1 = 500;
SET @rrt1CommotidyPrice1 = 300;
SET @rrt1AmuontCash = 150000.000000;
SET @rrt1AmuontWeChat = 0.000000;
SET @rrt1AmuontAliPay = 0.000000;
SET @rrt1TotalAmount = @rrt1CommotidyNO1 * @rrt1CommotidyPrice1;
-- 零售单收银汇总
...
VALUES (@staffIDA, 2,  @rggWorkTimeStart1, @rggWorkTimeEnd1, @rggNO1, @rggTotalAmount1, 90000.000000, @rggTotalAmountCash1, @rggTotalAmountWeChat1, @rggTotalAmountAliPay1, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
VALUES (@rtCommID1, @commIDA, @rtcs7NO, @warehouseA1);
SET @rtCommsourceIDA1 = last_insert_id();
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rtCommID1, @commIDA, @rtcs8NO, @warehouseA2);
...
-- 插入零售单商品来源
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rtCommID1, @commIDB, @rtcs1NO, @warehouseB1);
SET @rtCommsourceIDB1 = last_insert_id();
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rtCommID1, @commIDB, @rtcs2NO, @warehouseB2);
...
-- 插入零售单商品来源
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rtCommID3, @commIDC, @rtcs3NO, @warehouseC1);
SET @rtCommsourceIDC1 = last_insert_id();
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rtCommID3, @commIDC, @rtcs4NO, @warehouseC2);
SET @rtCommsourceIDC2 = last_insert_id();

-- 员工A新建退货单
INSERT INTO t_retailtrade (F_VipID, F_SN, F_LocalSN, F_POS_ID, F_Logo, F_SaleDatetime, F_StaffID, F_PaymentType, F_PaymentAccount, F_Status, F_Remark, F_SourceID, F_SyncDatetime, F_Amount, F_AmountCash, F_AmountAlipay, F_AmountWeChat, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_SmallSheetID, F_AliPayOrderSN, F_WxOrderSN, F_WxTradeNO, F_WxRefundNO, F_WxRefundDesc, F_WxRefundSubMchID, F_ShopID)
VALUES (1,'LS2041020200000500010001_1',11069,1,'url=ashasoadigmnalskd','2041/02/02 07:00:01',@staffIDA,1,'0',1,'…',@rtID1,'2041/02/02 07:00:01',@rrt1TotalAmount,@rrt1AmuontCash,@rrt1AmuontWeChat,@rrt1AmuontAliPay,0.000000,0.000000,0.000000,0.000000,0.000000,1,NULL,NULL,NULL,NULL,NULL,NULL,2);
SET @returnID2 = last_insert_id();
-- 零售商品A
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@returnID2, @commIDA, '普通商品A', 1, @rrt1CommotidyNO1, 321, 500, @rrt1CommotidyPrice1, 300, NULL);
SET @returnCommID1 = last_insert_id();
-- 插入零售单商品来源
-- INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
-- VALUES (@returnCommID1, @commIDA, @rtcs5NO, @warehouseA1);
-- SET @returnCommsourceIDA1 = last_insert_id();
-- INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
-- VALUES (@returnCommID1, @commIDA, @rtcs6NO, @warehouseA2);
...
VALUES (@returnCommID1, @commIDA, @rtcd1NO, @warehouseA2);
SET @returnAgg1 = last_insert_id();
INSERT INTO t_returnretailtradecommoditydestination(F_RetailTradeCommodityID, F_IncreasingCommodityID, F_NO, F_WarehousingID)
VALUES (@returnCommID1, @commIDA, @rtcd2NO, @warehouseA1);
SET @returnAgg2 = last_insert_id();
-- 
SET @iErrorCode = 0;
SET @sErrorMsg = '';
...
-- 售卖商品总成本
SET @saleTotalCost = (@rtcs7NO * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @warehouseA1 AND F_CommodityID = @commIDA))
					 +(@rtcs8NO * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @warehouseA2 AND F_CommodityID = @commIDA))
					 +(@rtcs1NO * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @warehouseB1 AND F_CommodityID = @commIDB))
					 +(@rtcs2NO * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @warehouseB2 AND F_CommodityID = @commIDB))
					 +(@rtcs3NO * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @warehouseC1 AND F_CommodityID = @commIDC))
					 +(@rtcs4NO * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @warehouseC2 AND F_CommodityID = @commIDC));
-- 售卖商品总毛利
SET @saleTotalMargin = @saleTotalAmount - @saleTotalCost;
-- 退货总金额
SET @returnTotalAmount = @rrt1CommotidyNO1 * @rrt1CommotidyPrice1;
-- 退货商品总成本
SET @returnTotalCost =(@rtcd1NO * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @warehouseA2 AND F_CommodityID = @commIDA))
					 +(@rtcd2NO * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @warehouseA1 AND F_CommodityID = @commIDA));

-- 退货商品总毛利
SELECT @returnTotalCost;
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification1 = 1,'测试成功','测试失败') AS 'Test Case20 Result';
-- 
...
DELETE FROM t_returnretailtradecommoditydestination WHERE F_ID = @returnAgg1;
DELETE FROM t_returnretailtradecommoditydestination WHERE F_ID = @returnAgg2;
