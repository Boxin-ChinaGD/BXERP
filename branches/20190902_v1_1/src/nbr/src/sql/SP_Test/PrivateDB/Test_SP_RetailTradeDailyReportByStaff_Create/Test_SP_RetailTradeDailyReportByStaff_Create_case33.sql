SELECT '----- case33:员工A创建一个零售单卖A商品，对应两个入库单 ,卖B商品，对应两个入库单 , 卖C商品，对应两个入库单,一张退货单，对商品A、B全部退货,对C部分退货 [A]-------' AS 'Case33';
-- 
-- 员工ID
SET @staffIDA = 9;
...
-- 新建退货单
SET @returnCommANO_A = @warehouseA1NO + @warehouseA2NO; -- 全退
SET @returnCommBNO_A = @warehouseB1NO + @warehouseB2NO; -- 全退
...
-- 零售商品B
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@rrtID_A, @commIDB, '普通商品B', 3, @returnCommBNO_A, @bCommotidyPrice, @returnCommBNO_A, @bCommotidyPrice, 200, NULL);
...
SET @rggTotalAmount = @rtTotalAmount_A;
SET @rggTotalAmountCash = @rtAmuontCash_A;
SET @rggTotalAmountWeChat = @rtAmuontWeChat_A;
SET @rggTotalAmountAliPay = @rtAmuontAliPay_A;
SET @rggNO = 1;
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case33 Result';
-- 
