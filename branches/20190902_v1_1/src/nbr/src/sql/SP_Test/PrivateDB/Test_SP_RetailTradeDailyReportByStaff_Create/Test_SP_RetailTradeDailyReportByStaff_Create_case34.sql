SELECT '----- case34:员工A创建一个零售单卖A商品，对应两个入库单 ,卖B商品，对应两个入库单 , 卖C商品，对应两个入库单,员工B创建一张退货单，对商品A、B全部退货、对C部分退货[AB]-------' AS 'Case34';
-- 
-- 员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
SET @warehouseA1Price = 11.32;
SET @warehouseA2Price = 15.11;
...
-- 插入入库单商品表
SET @warehouseC1NO = 27;
SET @warehouseC2NO = 23;
SET @warehouseC1Price = 12.12;
SET @warehouseC2Price = 13;
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
...
-- 员工B新建退货单
SET @returnCommANO_B = @warehouseA1NO + @warehouseA2NO; -- 全退
SET @returnCommBNO_B = @warehouseB1NO + @warehouseB2NO; -- 全退
...
VALUES (@staffIDA, 2, @rggWorkTimeStart, @rggWorkTimeEnd, @rggNO, @rggTotalAmount_A, 10.000000, @rggTotalAmountCash_A, @rggTotalAmountWeChat_A, @rggTotalAmountAliPay_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgID_A = last_insert_id();
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDB, 2, @rggWorkTimeStart, @rggWorkTimeEnd, @rggNO, @rggTotalAmount_B, 10.000000, @rggTotalAmount_B, @rggTotalAmountWeChat_B, @rggTotalAmountAliPay_B, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SELECT @sErrorMsg;
-- 结果验证
-- 验证员工A有零售有退货(B退A的单，A在上班，所以算A的业绩)
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case34 Result';
-- 
-- 结果验证
-- 验证员工B什么都没有(员工B退员工A的单，员工A在上班，所以不算员工B的业绩)
