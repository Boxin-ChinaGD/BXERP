SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case43.sql+++++++++++++++++++++++';

SELECT '----- case43:员工A创建一个零售单卖A商品，对应两个入库单 ,B商品，对应两个入库单 , C商品，对应两个入库单,员工B隔天创建一张退货单，对商品A、B、C全部退货[A、AB]-------' AS 'Case43';
-- 
-- 创建一个商品A
...
-- 插入入库单商品表
SET @warehouseA1NO = 501;
SET @warehouseA2NO = 503;
SET @warehouseA1Price = 11.11;
SET @warehouseA2Price = 12.21;
...
-- 插入入库单商品表
SET @warehouseB1NO = 11;
SET @warehouseB2NO = 47;
SET @warehouseB1Price = 10.760000;
SET @warehouseB2Price = 12.310000;
...
-- 插入入库单商品表
SET @warehouseC1NO = 331;
SET @warehouseC2NO = 253;
SET @warehouseC1Price = 17.56;
SET @warehouseC2Price = 21.89;
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
...
SET @returnCommCNO_B = @saleCommCNO_A; -- 全退
...
SET @saleDatetime_B = '2041/02/02 7:00:01';
SET @syncDatetime_B = '2041/02/02 7:00:01';
...
SET @returnID_B = last_insert_id();
-- 零售商品A
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@returnID_B, @commIDA, '普通商品A', @barcodeIDA, @returnCommANO_B, @aCommotidyPrice, @returnCommANO_B, @aCommotidyPrice, 300, NULL);
SET @returnCommAID_B = last_insert_id();
-- 零售商品B
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@returnID_B, @commIDB, '可口可乐', @barcodeIDB, @returnCommBNO_B, @bCommotidyPrice, @returnCommBNO_B, @bCommotidyPrice, 200, NULL);
SET @returnCommBID_B = last_insert_id();
-- 零售商品C
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@returnID_B, @commIDC, '普通商品C', @barcodeIDC, @returnCommCNO_B, @cCommotidyPrice, @returnCommCNO_B, @cCommotidyPrice, 700, NULL);
SET @returnCommCID_B = last_insert_id();
-- 插入退货去向表
...
INSERT INTO t_returnretailtradecommoditydestination(F_RetailTradeCommodityID, F_IncreasingCommodityID, F_NO, F_WarehousingID)
VALUES (@returnCommBID_B, @commIDB, @NOReturnToB2, @warehouseB2);
SET @rrtCommB2Destination_B = last_insert_id();
-- 商品C
INSERT INTO t_returnretailtradecommoditydestination(F_RetailTradeCommodityID, F_IncreasingCommodityID, F_NO, F_WarehousingID)
VALUES (@returnCommCID_B, @commIDC, @NOReturnToC1, @warehouseC1);
SET @rrtCommC1Destination_B = last_insert_id();
INSERT INTO t_returnretailtradecommoditydestination(F_RetailTradeCommodityID, F_IncreasingCommodityID, F_NO, F_WarehousingID)
VALUES (@returnCommCID_B, @commIDC, @NOReturnToC2, @warehouseC2);
SET @rrtCommC2Destination_B = last_insert_id();
-- 
-- 零售单收银汇总
-- 用于02/01员工A的业绩验证
...
SET @rggWorkTimeStart1='2041/02/01 00:00:00';
SET @rggWorkTimeEnd1='2041/02/01 23:59:59';
SET @rggNO = 1;
-- 
...
VALUES (@staffIDA, 2, @rggWorkTimeStart1, @rggWorkTimeEnd1, @rggNO, @rggTotalAmount_A1, 0.000000, @rggTotalAmountCash_A1, @rggTotalAmountWeChat_A1, @rggTotalAmountAliPay_A1, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgID1_A = last_insert_id();

-- 
SET @iErrorCode = 0;
SET @sErrorMsg = '';
SET @dSaleDatetimeYesterday = '2041-02-01 00:00:00';
SET @deleteOldData = 1; -- 仅测试使用 1，删除旧的数据。只能在测试代码中使用。0，不删除旧的数据。只能在功能代码中使用。
SET @iShopID = 2;
-- 
CALL SP_RetailTradeDailyReportByStaff_Create(@iErrorCode, @sErrorMsg, @iShopID, @dSaleDatetimeYesterday, @deleteOldData);
SELECT @iErrorCode;
SELECT @sErrorMsg;
-- 结果验证
-- 验证员工A在02/01当天只有零售
...
AND F_NO = (
	...
	AND datediff(F_SaleDatetime, @dSaleDatetimeYesterday) = 0
 );

SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case43 Result';

-- 验证员工A、B在02/02当天只有B进行退A零售单
SET @rggTotalAmount_A2 = @rrtTotalAmount_B;
SET @rggTotalAmountCash_A2 = @rrtAmuontCash_B;
SET @rggTotalAmountWeChat_A2 = @rrtAmuontWeChat_B;
SET @rggTotalAmountAliPay_A2 = @rrtAmuontAliPay_B;
SET @rggWorkTimeStart2='2041/02/02 00:00:00';
SET @rggWorkTimeEnd2='2041/02/02 23:59:59';
...
SET @rggNO = 1;
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDA, 2, @rggWorkTimeStart2, @rggWorkTimeEnd2, @rggNO, @rggTotalAmount_A2, 2400.000000, @rggTotalAmountCash_A2, @rggTotalAmountWeChat_A2, @rggTotalAmountAliPay_A2, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgID2_A = last_insert_id();
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDB, 2, @rggWorkTimeStart2, @rggWorkTimeEnd2, @rggNO, @rggTotalAmount_B, 2400.000000, @rggTotalAmountCash_B, @rggTotalAmountWeChat_B, @rggTotalAmountAliPay_B, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
-- 验证员工A在02/02当天只有退货
-- 售卖总金额
SET @saleTotalAmount_A2 = 0.000000;
-- 售卖商品总成本
SET @saleTotalCost_A2 = 0.000000;
-- 售卖商品总毛利
SET @saleTotalMargin_A2 = @saleTotalAmount_A2 - @saleTotalCost_A2;
-- 退货总金额
SET @returnTotalAmount_A2= @rrtTotalAmount_B;
-- 退货商品总成本
SET @returnTotalCost_A2 = (@NOReturnToA1 * @warehouseA1Price + @NOReturnToA2 * @warehouseA2Price) + 
						 (@NOReturnToB1 * @warehouseB1Price + @NOReturnToB2 * @warehouseB2Price) + 
						 (@NOReturnToC1 * @warehouseC1Price + @NOReturnToC2 * @warehouseC2Price);
-- 退货商品总毛利
SET @returnTotalMargin_A2 = @returnTotalAmount_A2 - @returnTotalCost_A2;
-- 总金额（售卖总金额 - 退货总金额）
SET @totalAmount_A2 = @saleTotalAmount_A2 - @returnTotalAmount_A2;
-- 总毛利（售卖商品总毛利 - 退货商品总毛利）
SET @grossMargin_A2 = @saleTotalMargin_A2 - @returnTotalMargin_A2;
SELECT @totalAmount_A2;
SELECT @grossMargin_A2;
SET @ResultVerification_A2=0;
-- 进行员工A业绩报表的数据结果验证
SELECT 1 INTO @ResultVerification_A2
FROM t_retailtradedailyreportbystaff 
WHERE F_StaffID = @staffIDA 
AND F_Datetime = @dSaleDatetime
AND F_TotalAmount = @totalAmount_A2
AND F_GrossMargin = @grossMargin_A2
AND F_NO = (
	SELECT COUNT(1) 
	FROM t_retailtrade 
	WHERE F_SourceID = -1 
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A2 = 1,'测试成功','测试失败') AS 'Test Case43 Result';
-- 
-- 验证员工B在02/02虽然有退员工A的单，但是员工A在上班，不算员工B的业绩
-- 售卖总金额
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case43 Result';
-- 
...
DELETE FROM t_returnretailtradecommoditydestination WHERE F_ID = @rrtCommB2Destination_B;
DELETE FROM t_returnretailtradecommoditydestination WHERE F_ID = @rrtCommC1Destination_B;
DELETE FROM t_returnretailtradecommoditydestination WHERE F_ID = @rrtCommC2Destination_B;
...
DELETE FROM t_retailtradecommodity WHERE F_ID = @returnCommCID_B;
DELETE FROM t_retailtrade WHERE F_ID = @rtID1_A;
DELETE FROM t_retailtrade WHERE F_ID = @returnID_B;
-- 商品A相关
