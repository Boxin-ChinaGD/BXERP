SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case48.sql+++++++++++++++++++++++';

SELECT '----- case48:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,员工B创建一张退货单，对商品A全部退货,对B部分退货 [AB]-------' AS 'Case48';
...
-- 插入入库单商品表
SET @warehouseB1NO = 250;
SET @warehouseB1Price = 17.030000;
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
...
SET @warehouseC1Price = 15.160000;
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
...
SET @returnCommBNO_B = 102; -- 部分退,总250
...
VALUES (@returnCommA1ID_B, @commIDA, @NOReturnToA1, @warehouseA1);
SET @rrtCommA1Destination_B = last_insert_id();
INSERT INTO t_returnretailtradecommoditydestination(F_RetailTradeCommodityID, F_IncreasingCommodityID, F_NO, F_WarehousingID)
VALUES (@returnCommA1ID_B, @commIDA, @NOReturnToA2, @warehouseA2);
SET @rrtCommA2Destination_B = last_insert_id();
-- 商品B
INSERT INTO t_returnretailtradecommoditydestination(F_RetailTradeCommodityID, F_IncreasingCommodityID, F_NO, F_WarehousingID)
VALUES (@returnCommB1ID_B, @commIDB, @NOReturnToB1, @warehouseB1);
...
VALUES (@staffIDA, 2, @rggWorkTimeStart_A, @rggWorkTimeEnd_A, @rggNO_A, @rggTotalAmount_A, 800.000000, @rggTotalAmountCash_A, @rggTotalAmountWeChat_A, @rggTotalAmountAliPay_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgID_A = last_insert_id();
-- 店员B退A的单，此时A、B都在上班，故该退货单不算在店员B业绩上且B未售卖
...
SET @rggWorkTimeStart_B='2041/02/02 06:00:00';
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case48 Result';
-- 
...
DELETE FROM t_retailtradecommodity WHERE F_ID = @returnCommA1ID_B;
DELETE FROM t_retailtradecommodity WHERE F_ID = @returnCommB1ID_B;
