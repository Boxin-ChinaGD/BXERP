SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case49.sql+++++++++++++++++++++++';

SELECT '----- case49:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,员工B创建一张退货单，对商品A全部退货,对B部分退货，上午卖，下午退 [AB]-------' AS 'Case49';
...
-- 插入入库单商品表
SET @warehouseB1NO = 212;
SET @warehouseB1Price = 13.220000;
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
...
-- 插入入库单商品表
SET @warehouseC1NO = 200;
SET @warehouseC1Price = 14.060000;
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
...
SET @bCommotidyPrice = 207.000000;
...
SET @returnCommBNO_B = 11; -- 部分退,总212
...
VALUES (@returnCommAID1, @commIDA, @NOReturnToA1, @warehouseA1);
SET @rrtCommA1Destination_B = last_insert_id();
INSERT INTO t_returnretailtradecommoditydestination(F_RetailTradeCommodityID, F_IncreasingCommodityID, F_NO, F_WarehousingID)
VALUES (@returnCommAID1, @commIDA, @NOReturnToA2, @warehouseA2);
SET @rrtCommA2Destination_B = last_insert_id();
-- 商品B
INSERT INTO t_returnretailtradecommoditydestination(F_RetailTradeCommodityID, F_IncreasingCommodityID, F_NO, F_WarehousingID)
VALUES (@returnCommBID1, @commIDB, @NOReturnToB1, @warehouseB1);
...
SET @rggWorkTimeStart_A='2041/02/02 06:00:00';
SET @rggWorkTimeEnd_A='2041/02/02 11:59:59';
SET @rggNO_A = 1;
...
VALUES (@staffIDA, 2, @rggWorkTimeStart_A, @rggWorkTimeEnd_A, @rggNO_A, @rggTotalAmount_A, 80.000000, @rggTotalAmountCash_A, @rggTotalAmountWeChat_A, @rggTotalAmountAliPay_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SET @rggWorkTimeStart2_A='2041/02/02 14:00:00';
SET @rggWorkTimeEnd2_A='2041/02/02 18:59:59';
SET @rggNO2_A = 0;
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDA, 2, @rggWorkTimeStart2_A, @rggWorkTimeEnd2_A, @rggNO2_A, @rggTotalAmount2_A, 100.000000, @rggTotalAmountCash2_A, @rggTotalAmountWeChat2_A, @rggTotalAmountAliPay2_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgID2_A = last_insert_id();
-- 店员B退A早上的单，此时A、B都在上班，故该退货单不算在店员B业绩上且B未售卖
...
SELECT @iErrorCode;
SELECT @sErrorMsg;
-- 验证员工A在02/02,员工A售卖一张单，被店员B进行了退货，且同时在上班
-- 售卖总金额
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case49 Result';
-- 验证员工B在02/02的业绩,只进行了店员A的单的退货，且A、B同时上班
-- 售卖总金额
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case49 Result';
-- 
...
DELETE FROM t_retailtradecommodity WHERE F_ID = @returnCommAID1;
DELETE FROM t_retailtradecommodity WHERE F_ID = @returnCommBID1;
