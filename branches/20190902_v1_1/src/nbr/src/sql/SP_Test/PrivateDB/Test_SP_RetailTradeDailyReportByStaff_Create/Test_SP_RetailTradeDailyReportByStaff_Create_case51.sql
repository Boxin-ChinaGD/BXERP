SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case51.sql+++++++++++++++++++++++';

SELECT '----- case51:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,员工B创建一张退货单，对商品A全部退货,对B部分退货，昨天卖(只有A上班,只售卖无退货)，今天退(A和B都在上班,只退货无售卖) [A、AB]-------' AS 'Case51';
...
-- 商品信息
SET @commodityCreateTime = '2119-01-01 09:00:00'; -- 商品的创建时间
SET @warehousingTime = '2119-01-01 06:00:00'; -- 商品入库时间
SET @commodityPriceA = 330; 
SET @warehousingCommodityA_Price1 = 5; -- 入库单1
SET @warehousingCommodityA_NO1 = 500;
SET @warehousingCommodityA_Price2 = 20; -- 入库单2
SET @warehousingCommodityA_NO2 = 500;
SET @commodityPriceB = 220;
SET @warehousingCommodityB_Price = 17;
SET @warehousingCommodityB_NO = 500;
SET @commodityPriceC = 770;
SET @warehousingCommodityC_Price = 30;
SET @warehousingCommodityC_NO = 500;

-- 零售单1
SET @rt1SaleDatetimeA = '2041/02/01 06:00:00'; -- 创建零售单时间
SET @rt1CommotidyNO1Yesterday_A = 150; -- 入库单1
SET @rt1CommotidyNO2Yesterday_A = 320; -- 入库单2
SET @rt1CommotidyPriceYesterday_A = 222;
SET @rt1CommotidyNOYesterday_B = 57;
SET @rt1CommotidyPriceYesterday_B = 132;
SET @rt1CommotidyNOYesterday_C = 98;
SET @rt1CommotidyPriceYesterday_C = 263;
...
SET @rrt1CommotidyNO_B = @rt1CommotidyNOYesterday_B - ROUND(@rt1CommotidyNOYesterday_B * 0.5); -- 退50%的数量
...
-- 创建收银员A今天的收银汇总
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffID_A, 2, @rggWorkTimeStart_A, @rggWorkTimeEnd_A, @rggNO_A, 0 - @rrt1TotalAmount_B, @ReserveAmount, @rggTotalAmountCash_A, @rggTotalAmountWeChat_A, @rggTotalAmountAliPay_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
VALUES (@warehouseA1, @commIDA, @warehousingCommodityA_NO1, 3, '跨库商品A', @barcodeIDA, @warehousingCommodityA_Price1, (@warehousingCommodityA_NO1 * @warehousingCommodityA_Price1), -- 
...
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, -- 
F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
VALUES (@warehouseA2, @commIDA, @warehousingCommodityA_NO2, 3, '跨库商品A', @barcodeIDA, @warehousingCommodityA_Price2, (@warehousingCommodityA_NO2 * @warehousingCommodityA_Price2), -- 
@warehousingTime, 36, @warehousingTime, @warehousingTime, @warehousingTime, 5);
...
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, -- 
F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
VALUES (@warehouseB, @commIDB, @warehousingCommodityB_NO, 3, '跨库商品B', @barcodeIDB, @warehousingCommodityB_Price, (@warehousingCommodityB_NO * @warehousingCommodityB_Price), -- 
@warehousingTime, 36, @warehousingTime, @warehousingTime, @warehousingTime, 5);
...
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, -- 
F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
VALUES (@warehouseC, @commIDC, (@warehousingCommodityC_NO * @warehousingCommodityC_Price), 3, '跨库商品C', @barcodeIDC, @warehousingCommodityC_Price, (@warehousingCommodityC_NO * @warehousingCommodityC_Price), -- 
@warehousingTime, 36, @warehousingTime, @warehousingTime, @warehousingTime, 5);
...
SELECT IF(@iErrorCode = 0, '测试成功', '测试失败') AS 'Case51.1 Testing Result';
-- 
...
SELECT IF(@countDefaultData = 1, '测试成功', '测试失败') AS 'Case51.2 Testing Result';
SET @totalAmountA = 0;
...
SELECT IF(@totalAmountA = 0 - @rrt1TotalAmount_B, '测试成功', '测试失败') AS 'Case51.3 Testing Result';
-- 
...
SELECT IF(@retailtradeNOA = @rtNoByStaffA, '测试成功', '测试失败') AS 'Case51.4 Testing Result';
-- 
...
SELECT IF(@grossMargin = (@rrt1CommotidyNO1_A * @warehousingCommodityA_Price1) + (@rrt1CommotidyNO2_A * @warehousingCommodityA_Price2) -- 
						+ (@rrt1CommotidyNO_B * @warehousingCommodityB_Price) - @rrt1TotalAmount_B , '测试成功', '测试失败') AS 'Case51.5 Testing Result';
-- 
...
SELECT IF(@countDefaultDataB = 1, '测试成功', '测试失败') AS 'Case51.6 Testing Result';
SET @totalAmountB = 0;
...
SELECT IF(@totalAmountB = 0.000000, '测试成功', '测试失败') AS 'Case51.7 Testing Result';
...
SELECT IF(@retailtradeNOB = @rtNoByStaffB, '测试成功', '测试失败') AS 'Case51.8 Testing Result';
...
SELECT IF(@grossMarginB = 0.000000, '测试成功', '测试失败') AS 'Case51.9 Testing Result';
-- 删除测试创建的数据
