SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case53.sql+++++++++++++++++++++++';

SELECT '----- case53:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,创建一张退货单，对商品A、B、C全部退货[A]-------' AS 'Case53';
-- 收银员
SET @staffID_A = 9;
...
SET @warehousingCommodityA_Price1 = 9.95; -- 入库单1
SET @warehousingCommodityA_NO1 = 500;
SET @warehousingCommodityA_Price2 = 8.99; -- 入库单2
SET @warehousingCommodityA_NO2 = 500;
SET @commodityPriceB = 220;
SET @warehousingCommodityB_Price = 7.77;
SET @warehousingCommodityB_NO = 500;
SET @commodityPriceC = 770;
SET @warehousingCommodityC_Price = 6.66;
SET @warehousingCommodityC_NO = 500;

-- 零售单1
SET @rt1SaleDatetimeA = '2041/02/02 06:00:00'; -- 创建零售单时间
SET @rt1CommotidyNO1_A = 33; -- 入库单1
SET @rt1CommotidyNO2_A = 22; -- 入库单2
SET @rt1CommotidyPrice_A = 99.9;
SET @rt1CommotidyNO_B = 55;
SET @rt1CommotidyPrice_B = 77.7;
SET @rt1CommotidyNO_C = 66;
SET @rt1CommotidyPrice_C = 66.6;
...
SET @rt1AmuontCash_A = (@rt1CommotidyNO1_A + @rt1CommotidyNO2_A) * @rt1CommotidyPrice_A;
SET @rt1AmuontWeChat_A = (@rt1CommotidyNO_B * @rt1CommotidyPrice_B) + (@rt1CommotidyNO_C * @rt1CommotidyPrice_C);
...
SET @rggTotalAmountWeChat_A = @rrt1AmuontWeChat_B;
SET @rggTotalAmountAliPay_A = @rrt1AmuontAliPay_B;
SET @rggNO_A = 1;						
...
VALUES (1,'LS2041020200000500010001_1',11069,1,'url=ashasoadigmnalskd',@rrt1SaleDatetimeB,@staffID_A,1,'0',1,'…',@rtIDA,@rrt1SaleDatetimeB,@rrt1TotalAmount_A,@rrt1AmuontCash_A,@rrt1AmuontAliPay_B,@rrt1AmuontWeChat_B,0.000000,0.000000,0.000000,0.000000,0.000000,1,NULL,NULL,NULL,NULL,NULL,NULL,2);
...
SELECT IF(@iErrorCode = 0, '测试成功', '测试失败') AS 'Case53.1 Testing Result';
-- 
...
SELECT IF(@countDefaultData = 1, '测试成功', '测试失败') AS 'Case53.2 Testing Result';
SET @totalAmountA = 0;
...
SELECT IF(@totalAmountA = 0.0000, '测试成功', '测试失败') AS 'Case53.3 Testing Result';
-- 
...
SELECT IF(@retailtradeNOA = @rtNoByStaffA, '测试成功', '测试失败') AS 'Case53.4 Testing Result';
-- 
-- 进行业绩报表的毛利结果验证
