SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case54.sql+++++++++++++++++++++++';

SELECT '----- case54:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,创建一张退货单，对商品A、B、C全部退货,上午卖，下午退 [A、A]-------' AS 'Case54';
-- 收银员
SET @staffID_A = 9;
...
SET @warehousingCommodityA_Price1 = 8.5; -- 入库单1
SET @warehousingCommodityA_NO1 = 500;
SET @warehousingCommodityA_Price2 = 5.55; -- 入库单2
SET @warehousingCommodityA_NO2 = 500;
SET @commodityPriceB = 220;
SET @warehousingCommodityB_Price = 3.33;
SET @warehousingCommodityB_NO = 500;
SET @commodityPriceC = 770;
SET @warehousingCommodityC_Price = 2.22;
SET @warehousingCommodityC_NO = 500;

-- 零售单1(上午售卖,下午全部退货)
SET @rt1SaleDatetimeA = '2041/02/02 06:00:00'; -- 创建零售单时间
SET @rt1CommotidyNO1_A = 99; -- 入库单1
SET @rt1CommotidyNO2_A = 88; -- 入库单2
SET @rt1CommotidyPrice_A = 177;
SET @rt1CommotidyNO_B = 77;
SET @rt1CommotidyPrice_B = 256;
SET @rt1CommotidyNO_C = 66;
SET @rt1CommotidyPrice_C = 685;
...
-- 收银员A上午的收银汇总信息(只售卖)
SET @rggWorkTimeStart_A.M='2041/02/02 00:00:00';
SET @rggWorkTimeEnd_A.M='2041/02/02 12:00:00';
SET @rggTotalAmount_A.M = @rt1TotalAmount_A; 
SET @rggTotalAmountCash_A.M = @rt1AmuontCash_A;
SET @rggTotalAmountWeChat_A.M = @rt1AmuontWeChat_A;
SET @rggTotalAmountAliPay_A.M = @rt1AmuontAliPay_A;
SET @rggNOYesterday_A.M = 1;
...
-- 收银员A下午的收银汇总信息(无售卖无退货)
SET @rggWorkTimeStart_P.M='2041/02/02 14:00:000';
SET @rggWorkTimeEnd_P.M='2041/02/02 23:59:59';
SET @rggTotalAmount_P.M = 0 -  @rrt1TotalAmount_A; 
SET @rggTotalAmountCash_P.M = 0 - @rrt1AmuontCash_A;
SET @rggTotalAmountWeChat_P.M = 0.000000;
SET @rggTotalAmountAliPay_P.M = 0.000000;
SET @rggNO_P.M = 0;
...
VALUES (@staffID_A, 2, @rggWorkTimeStart_A.M, @rggWorkTimeEnd_A.M, @rggNOYesterday_A.M, @rggTotalAmount_A.M, @ReserveAmount, @rggTotalAmountCash_A.m, @rggTotalAmountWeChat_A.M, @rggTotalAmountAliPay_A.M, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgIDA1 = last_insert_id();
-- 员工A下午的收银汇总
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffID_A, 2, @rggWorkTimeStart_P.M, @rggWorkTimeEnd_P.M, @rggNO_P.M , @rggTotalAmount_P.M, @ReserveAmount, @rggTotalAmountCash_P.M, @rggTotalAmountWeChat_P.M, @rggTotalAmountAliPay_P.M, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgIDA2 = last_insert_id();
-- 创建商品A
...
@rt1SaleDatetimeA, @rt1TotalAmount_A, @rt1AmuontCash_A, @rt1AmuontWeChat_A,@rt1AmuontWeChat_A,0.000000,0.000000,0.000000,0.000000,0.000000,1,NULL, -- 
...
-- 创建零售退货单商品C
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@returnIDB, @commIDC, '普通商品C', @barcodeIDC, @rrt1CommotidyNO_B, @commodityPriceC, @rrt1CommotidyNO_B, @rt1CommotidyPrice_C, 700, NULL);
...
SELECT IF(@iErrorCode = 0, '测试成功', '测试失败') AS 'Case54.1 Testing Result';
-- 
...
SELECT IF(@countDefaultData = 1, '测试成功', '测试失败') AS 'Case54.2 Testing Result';
SET @totalAmountA = 0;
...
SELECT IF(@totalAmountA = 0.000000, '测试成功', '测试失败') AS 'Case54.3 Testing Result';
-- 
-- 验证零售单数，统计总的零售单和员工业绩报表的F_NO是否一致
SET @retailtradeNOA = 0;
SELECT count(1) INTO @retailtradeNOA FROM t_retailtrade WHERE F_SourceID = -1 AND F_StaffID = @staffID_A AND F_SaleDatetime BETWEEN @rggWorkTimeStart_A.M AND @rggWorkTimeEnd_P.M;
SET @rtNoByStaffA = 0;
...
SELECT IF(@retailtradeNOA = @rtNoByStaffA, '测试成功', '测试失败') AS 'Case54.4 Testing Result';
-- 
-- 进行业绩报表的毛利结果验证
-- 销售毛利 = 销售总额 - 进货价 = 500 * 300 + 100 * 200 + 100 * 700 - (200 * 10 + 300 * 15)- 100*10 - 100*10 = 150000+20000+70000-6500-1000-1000=240000-8500=231500
-- 退货毛利 = 退货总额 - 进货价 = 160000 - (200 * 10 + 300 * 15) - 100*10 = 160000 - 6500 - dSaleDatetime;
...
SELECT IF(@grossMargin = 0, '测试成功', '测试失败') AS 'Case53.5 Testing Result';
-- 删除测试创建的数据
DELETE FROM t_retailtradedailyreportbystaff WHERE F_Datetime = @dSaleDatetime;
DELETE FROM t_retailtradeaggregation WHERE F_ID = @rtgIDA1;
DELETE FROM t_retailtradeaggregation WHERE F_ID = @rtgIDA2;
