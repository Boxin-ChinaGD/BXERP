SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case55.sql+++++++++++++++++++++++';

SELECT '----- case55:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,创建一张退货单，对商品A、B、C全部退货,昨天买，今天退[A]-------' AS 'Case55';
-- 收银员
SET @staffID_A = 9;
...
SET @warehousingCommodityA_Price1 = 12.3456; -- 入库单1
SET @warehousingCommodityA_NO1 = 99;
SET @warehousingCommodityA_Price2 = 6.54321; -- 入库单2
SET @warehousingCommodityA_NO2 = 88;
SET @commodityPriceB = 220;
SET @warehousingCommodityB_Price = 9.87654;
SET @warehousingCommodityB_NO = 77;
SET @commodityPriceC = 770;
SET @warehousingCommodityC_Price = 1.47852;
SET @warehousingCommodityC_NO = 66;

-- 零售单1
SET @rt1SaleDatetimeA = '2041/02/01 06:00:00'; -- 创建零售单时间
SET @rt1CommotidyNO1Yesterday_A = 77; -- 入库单1
SET @rt1CommotidyNO2Yesterday_A = 66; -- 入库单2
SET @rt1CommotidyPriceYesterday_A = 300;
SET @rt1CommotidyNOYesterday_B = 55;
SET @rt1CommotidyPriceYesterday_B = 200;
SET @rt1CommotidyNOYesterday_C = 44;
SET @rt1CommotidyPriceYesterday_C = 700;
...
SET @rrt1CommotidyNO_C = @rt1CommotidyNOYesterday_C;
SET @rrt1TotalAmount_A = (@rrt1CommotidyNO1_A + @rrt1CommotidyNO2_A) * @rt1CommotidyPriceYesterday_A -- 
						+ (@rrt1CommotidyNO_B * @rt1CommotidyPriceYesterday_B) + (@rrt1CommotidyNO_C * @rt1CommotidyPriceYesterday_C);
...
VALUES (@staffID_A, 2, @rggWorkTimeStartYesterday_A, @rggWorkTimeEndYesterday_A, @rggNOYesterday_A, @rt1TotalAmountYesterday_A, @ReserveAmount, @rt1AmuontCashYesterday_A, @rt1AmuontWeChatYesterday_A, @rt1AmuontAliPayYesterday_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, -- 
F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
VALUES (@warehouseB, @commIDB, (@warehousingCommodityB_Price * @warehousingCommodityB_NO), 3, '跨库商品B', @barcodeIDB, @warehousingCommodityB_Price, (@warehousingCommodityB_Price * @warehousingCommodityB_NO), -- 
@warehousingTime, 36, @warehousingTime, @warehousingTime, @warehousingTime, 5);
...
-- 创建零售退货单商品C
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@returnIDB, @commIDC, '普通商品C', @barcodeIDC, @rrt1CommotidyNO_C, @commodityPriceC, @rrt1CommotidyNO_C, @rt1CommotidyPriceYesterday_C, 700, NULL);
...
SELECT IF(@iErrorCode = 0, '测试成功', '测试失败') AS 'Case55.1 Testing Result';
-- 
...
SELECT IF(@countDefaultData = 1, '测试成功', '测试失败') AS 'Case55.2 Testing Result';
SET @totalAmountA = 0;
...
SELECT IF(@totalAmountA = 0 - @rrt1TotalAmount_A, '测试成功', '测试失败') AS 'Case55.3 Testing Result';
-- 
...
SELECT IF(@retailtradeNOA = @rtNoByStaffA, '测试成功', '测试失败') AS 'Case55.4 Testing Result';
-- 
-- 进行业绩报表的毛利结果验证
...
SELECT IF(@grossMargin = (@rrt1CommotidyNO1_A * @warehousingCommodityA_Price1) + (@rrt1CommotidyNO2_A * @warehousingCommodityA_Price2) -- 
			+ (@rrt1CommotidyNO_B * @warehousingCommodityB_Price) + (@rrt1CommotidyNO_C * @warehousingCommodityC_Price) - @rrt1TotalAmount_A, '测试成功', '测试失败') AS 'Case55.5 Testing Result';
-- 删除测试创建的数据
