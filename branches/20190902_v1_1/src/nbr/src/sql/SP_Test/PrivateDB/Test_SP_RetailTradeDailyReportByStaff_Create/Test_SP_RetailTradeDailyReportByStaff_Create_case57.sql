SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case57.sql+++++++++++++++++++++++';

SELECT '----- case57:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,员工B创建一张退货单，对商品A、B、C全部退货上午卖，下午退 [AB]-------' AS 'Case57';
-- 收银员
SET @staffID_A = 9;
SET @staffID_B = 10;
...
SET @warehousingCommodityA_Price1 = 8.88; -- 入库单1
SET @warehousingCommodityA_NO1 = 500;
SET @warehousingCommodityA_Price2 = 9.99; -- 入库单2
...
SET @rt1CommotidyNO1_A = 199; -- 入库单1
SET @rt1CommotidyNO2_A = 299; -- 入库单2
SET @rt1CommotidyPrice_A = 88.8;
SET @rt1CommotidyNO_B = 399;
SET @rt1CommotidyPrice_B = 77.7;
SET @rt1CommotidyNO_C = 499;
...
SELECT IF(@iErrorCode = 0, '测试成功', '测试失败') AS 'Case57.1 Testing Result';
-- 
...
SELECT IF(@countDefaultData = 1, '测试成功', '测试失败') AS 'Case57.2 Testing Result';
SET @totalAmountA = 0;
...
SELECT IF(@totalAmountA = 0.0000, '测试成功', '测试失败') AS 'Case57.3 Testing Result';
-- 
...
SELECT IF(@retailtradeNOA = @rtNoByStaffA, '测试成功', '测试失败') AS 'Case57.4 Testing Result';
-- 
-- 进行业绩报表的毛利结果验证
...
SELECT IF(@grossMargin = 0, '测试成功', '测试失败') AS 'Case57.5 Testing Result';
-- 
...
SELECT IF(@countDefaultDataB = 1, '测试成功', '测试失败') AS 'Case57.6 Testing Result';
SET @totalAmountB = 0;
...
SELECT IF(@totalAmountB = 0.000000, '测试成功', '测试失败') AS 'Case57.7 Testing Result';
...
SELECT IF(@retailtradeNOB = @rtNoByStaffB, '测试成功', '测试失败') AS 'Case57.8 Testing Result';
...
SELECT IF(@grossMarginB = 0.000000, '测试成功', '测试失败') AS 'Case57.9 Testing Result';
-- 删除测试创建的数据
