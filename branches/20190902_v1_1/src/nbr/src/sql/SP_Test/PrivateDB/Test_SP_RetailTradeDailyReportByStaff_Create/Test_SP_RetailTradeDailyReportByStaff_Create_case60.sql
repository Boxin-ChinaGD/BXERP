SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case60.sql+++++++++++++++++++++++';


SELECT '----- case60:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,员工B创建一张退货单，对商品A、B、C全部退货,昨天卖，今天退 [A、B]-------' AS 'Case60';
-- 收银员 
SET @staffID_A = 9;
SET @staffID_B = 10;
...
SET @rrt1TotalAmount_B = (@rrt1CommotidyNO1_A + @rrt1CommotidyNO2_A) * @rt1CommotidyPriceYesterday_A -- 
...
SET @rggTotalAmountCash_B = 0 - @rrt1TotalAmount_B;
...
-- 创建收银员B的收银汇总
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffID_B, 2, @rggWorkTimeStart_B, @rggWorkTimeEnd_B, @rggNO_B, @rggTotalAmount_B, @ReserveAmount, @rggTotalAmountCash_B, @rggTotalAmountWeChat_B, @rggTotalAmountAliPay_B = 0, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SELECT IF(@iErrorCode = 0, '测试成功', '测试失败') AS 'Case60.1 Testing Result';
-- 
...
SELECT IF(@countDefaultDataB = 1, '测试成功', '测试失败') AS 'Case60.2 Testing Result';
SET @totalAmountB = 0;
...
SELECT IF(@totalAmountB = 0 - @rrt1TotalAmount_B, '测试成功', '测试失败') AS 'Case60.3 Testing Result';
...
SELECT IF(@retailtradeNOB = @rtNoByStaffB, '测试成功', '测试失败') AS 'Case60.4 Testing Result';
...
SELECT IF(@grossMarginB = (@rt1CommotidyNO1Yesterday_A * @warehousingCommodityA_Price1) + (@rt1CommotidyNO2Yesterday_A * @warehousingCommodityA_Price2) -- 
						+ (@rt1CommotidyNOYesterday_B * @warehousingCommodityB_Price) + (@rt1CommotidyNOYesterday_C * @warehousingCommodityC_Price) - @rrt1TotalAmount_B, '测试成功', '测试失败') AS 'Case60.5 Testing Result';
-- 删除测试创建的数据
