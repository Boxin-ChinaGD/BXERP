SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case70.sql+++++++++++++++++++++++';

SELECT '----- case70:  员工A创建一个零售单卖A商品，商品A为未入库商品 ,B商品，对应一个入库单，对A全部退货，对商品B部分退货 [A] -------' AS 'Case70';	
-- 商品表
...
SET @rt1ReturnCommotidyNO1_B = 5;
SET @rt1ReturnCommotidyPrice1_B = @commodityPrice1_B;
SET @rt1ReturnAmuontCash = 200.000000;
SET @rt1ReturnAmuontWeChat = 0.000000;
SET @rt1ReturnAmuontAliPay = 0.000000;
SET @rt1ReturnTotalAmount = @rt1ReturnCommotidyNO1_A * @rt1ReturnCommotidyPrice1_A + @rt1ReturnCommotidyNO1_B * @rt1ReturnCommotidyPrice1_B;
-- 零售单收银汇总
...
SET @iStaffID = 9;
-- 入库商品表
SET @warehousingCommodityNO1_B = 500;
SET @warehousingCommodityPrice1_B = 8;
SET @warehousingCommodityAmount1_B = @warehousingCommodityNO1_B * @warehousingCommodityPrice1_B;
SET @warehousingCommodityNOSaleable1_B = 5;
-- 零售单商品来源表
...
-- 退货去向表
SET @rtcdNO2 = @rt1ReturnCommotidyNO1_B;
SET @rtcdWarehousingID2 = @warehouse1_B;
...
INSERT INTO T_RetailTradeCommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer)
VALUES (@rtID1, @commIDB,'未入库商品B', @barcodeIDB, @rt1CommotidyNO1_B, @rt1CommotidyPrice1_B, @rt1CommotidyNO1_B, @rt1CommotidyPrice1_B, @rt1CommotidyPrice1_B);
...
INSERT INTO T_RetailTradeCommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer)
VALUES (@rtReturnID1, @commIDB,'未入库商品B', @barcodeIDB, @rt1ReturnCommotidyNO1_B, @rt1ReturnCommotidyPrice1_B, 0, 0, @rt1ReturnCommotidyPrice1_B);
SET @rtcReturnID2 = last_insert_id();
...
SELECT IF(@saleTotalCost = (@warehousingCommodityPrice1_B * @rtcsNO2) ,'测试成功','测试失败') AS 'Test Case1 Result';
-- 售卖商品总毛利
...
SET @returnTotalCost = IFNULL((@rtcdNO1 * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @rtcdWarehousingID1 AND F_CommodityID = @commIDA)),0)
						+ IFNULL((@rtcdNO2 * (SELECT F_Price FROM t_warehousingcommodity WHERE F_WarehousingID = @rtcdWarehousingID2 AND F_CommodityID = @commIDA)),0);
-- 退货商品总毛利
...
DELETE FROM T_RetailTradeCommodity WHERE F_ID = @rtcReturnID2;
