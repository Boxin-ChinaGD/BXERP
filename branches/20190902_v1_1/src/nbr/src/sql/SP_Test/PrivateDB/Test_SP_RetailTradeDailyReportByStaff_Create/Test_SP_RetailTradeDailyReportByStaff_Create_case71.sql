SELECT '----- case71:  员工A创建一个零售单卖A商品，商品A为未入库商品 ,B商品，对应一个入库单，对A全部退货，对商品B全部退货 [A] -------' AS 'Case71';
-- 
-- 员工ID
SET @staffIDA = 9;
...
-- 插入入库单商品表
SET @warehouseB1NO = 500;
SET @warehouseBPrice = 12.542;
...
VALUES (@warehouseB, @commIDB, @warehouseB1NO, 3, '跨库商品B', @barcodeIDB, @warehouseBPrice, 5000, '2119-01-01 06:00:00', 36, '2119-01-09 06:00:00', '2119-01-01 06:00:00', '2119-01-01 06:00:00', 5);
SET @warehouseCommB = last_insert_id();

-- 插入零售单
SET @saleCommANO_A = 500; -- 未入库，直接卖500个
SET @aCommotidyPrice = 200.000000;
SET @saleCommBNO_A = 100;	-- 入库500，卖100个
...
VALUES (@staffIDA, 2, @rggWorkTimeStart_A, @rggWorkTimeEnd_A, 1, @rggTotalAmount_A, 10.000000, @rggTotalAmountCash_A, @rggTotalAmountWeChat_A, @rggTotalAmountAliPay_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case71 Result';
-- 
