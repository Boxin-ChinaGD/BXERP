SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case75.sql+++++++++++++++++++++++';

SELECT '----- case75:  员工A创建一个零售单卖A商品，商品A为未入库商品 ,卖B商品，对应一个入库单，员工B创建退货单，对A全部退货，对商品B部分退货 [AB] -------' AS 'Case75';
-- 
--  员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
-- 创建零售单
SET @saleCommANO_A = 202; -- 未入库，直接卖500
SET @aCommotidyPrice = 123.230000;
SET @saleCommBNO_A = 205;	-- 入库500，卖100
SET @bCommotidyPrice = 330.020000;
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case75 Result';
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case75 Result';
-- 
