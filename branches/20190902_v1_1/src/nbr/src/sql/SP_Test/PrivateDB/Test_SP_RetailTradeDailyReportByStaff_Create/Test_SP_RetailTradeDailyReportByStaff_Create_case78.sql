SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case78.sql+++++++++++++++++++++++';

SELECT '----- case78:  员工A创建一个零售单卖A商品，商品A为未入库商品 ,B商品，对应一个入库单，员工B创建退货单，对A部分退货，对商品B部分退货 [A、B] -------' AS 'Case78';
-- 
-- 员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
-- 插入入库单商品表
SET @warehouseBNO = 500; -- 入库数量500
SET @warehouseBPrice = 11; -- 入库价10
...
SET @aCommotidyPrice = 231.540000;
SET @saleCommBNO_A = 100;	-- 入库500，卖100
SET @bCommotidyPrice = 152.340000;
...
VALUES (@rtCommAID_A, @commIDB, @saleCommBNO_A, @warehouseB1);
SET @rtCommsourceIDB_A = last_insert_id();
-- 
-- 员工B新建退货单
SET @returnCommANO_B = 250; -- 部分退，退250
...
SET @saleDatetime2_B = '2041/02/02 16:00:01';

INSERT INTO t_retailtrade (F_VipID, F_SN, F_LocalSN, F_POS_ID, F_Logo, F_SaleDatetime, F_StaffID, F_PaymentType, F_PaymentAccount, F_Status, F_Remark, F_SourceID, F_SyncDatetime, F_Amount, F_AmountCash, F_AmountAlipay, F_AmountWeChat, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_SmallSheetID, F_AliPayOrderSN, F_WxOrderSN, F_WxTradeNO, F_WxRefundNO, F_WxRefundDesc, F_WxRefundSubMchID, F_ShopID)
VALUES (1,'LS2041020200000500010001_1',11069,1,'url=ashasoadigmnalskd',@saleDatetime2_B,@staffIDB,1,'0',1,'…',@rtID_A,'2041/02/02 17:00:01',@rrtTotalAmount_B,@rrtAmuontCash_B,@rrtAmuontAliPay_B,@rrtAmuontWeChat_B,0.000000,0.000000,0.000000,0.000000,0.000000,1,NULL,NULL,NULL,NULL,NULL,NULL,2);
...
VALUES (@returnCommBID_B, @commIDB, @returnCommBNO_B, @warehouseB1);
SET @returnCommBDestn_B = last_insert_id();
-- 
-- 零售汇总
SET @rgg1WorkTimeStart_A ='2041/02/02 00:00:00';
SET @rgg1WorkTimeEnd_A ='2041/02/02 12:00:00';
SET @rgg1TotalAmountCash_A = @rtAmuontCash_A;
SET @rgg1TotalAmountWeChat_A = @rtAmuontWeChat_A;
SET @rgg1TotalAmountAliPay_A = @rtAmuontAliPay_A;
SET @rgg1TotalAmount_A = @rgg1TotalAmountCash_A + @rgg1TotalAmountWeChat_A + @rgg1TotalAmountAliPay_A;
-- 
SET @rgg2WorkTimeStart_B ='2041/02/02 14:00:00';
SET @rgg2WorkTimeEnd_B ='2041/02/02 23:59:59';
SET @rgg2TotalAmountCash_B = @rrtAmuontCash_B;
SET @rgg2TotalAmountWeChat_B = @rrtAmuontWeChat_B;
SET @rgg2TotalAmountAliPay_B = @rrtAmuontAliPay_B;
...
VALUES (@staffIDA, 2, @rgg1WorkTimeStart_A, @rgg1WorkTimeEnd_A, 1, @rgg1TotalAmount_A, 100.000000, @rgg1TotalAmountCash_A, @rgg1TotalAmountWeChat_A, @rgg1TotalAmountAliPay_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgID_A = last_insert_id();
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDB, 2, @rgg2WorkTimeStart_B, @rgg2WorkTimeEnd_B, 1, @rgg2TotalAmount_B, 100.000000, @rgg2TotalAmountCash_B, @rgg2TotalAmountWeChat_B, @rgg2TotalAmountAliPay_B, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case78 Result';
...
-- 退货总金额
SET @returnTotalAmount_B = @rgg2TotalAmount_B;
-- 退货商品总成本
SET @returnTotalCost_B = @returnCommBNO_B * @warehouseBPrice;
-- 退货商品总毛利
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case78 Result';
-- 
