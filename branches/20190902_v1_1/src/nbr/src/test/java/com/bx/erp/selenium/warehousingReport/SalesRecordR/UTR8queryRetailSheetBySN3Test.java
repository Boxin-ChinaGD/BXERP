...
import java.util.*;
public class UTR8queryRetailSheetBySN3Test {
  ...
  @Test
  public void uTR8queryRetailSheetBySN3() {
    LoginSucceed();
...
//    System.out.println("销售记录的条数为：vars.get("counts").toString()");
    ...
    driver.findElement(By.xpath("//div[@id=\'purchasingOrderMain\']/div/div/div[4]/input")).sendKeys(" ");
    driver.findElement(By.cssSelector(".layui-icon-search")).click();
    if ((Boolean) js.executeScript("return (document.getElementsByName(\'layui-laypage-count\').value == \'\')")) {
      System.out.println("无数据");
    } else {
      assertThat(driver.findElement(By.cssSelector("#allSalesNoteList + div .layui-laypage-count")).getText(), is("vars.get(\"counts\").toString()"));
      // TODO
//      System.out.println("销售记录的条数为：vars.get("counts").toString()");
    }
    driver.close();
