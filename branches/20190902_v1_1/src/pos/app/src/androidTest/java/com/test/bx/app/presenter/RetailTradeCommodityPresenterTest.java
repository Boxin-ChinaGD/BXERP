...
import com.bx.erp.event.RetailTradeCommoditySQLiteEvent;
...
public class RetailTradeCommodityPresenterTest extends BaseAndroidTestCase {


    private static RetailTradeCommodityPresenter rtcp;
...
//    private static final long Timeout = 30 * 1000;
    ...
    public void setUp() throws Exception {
        ...
        rtcp = GlobalController.getInstance().getRetailTradeCommodityPresenter();
    ...
    public void test_a_getMaxId() throws CloneNotSupportedException {
        ...
        text = (RetailTradeCommodity) rtcp.createObjectSync(BaseSQLiteBO.INVALID_CASE_ID, text);
        Assert.assertTrue("错误码不正确！", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);

        long maxID = rtcp.generateTmpRowID(BaseModel.FIELD_NAME_syncDatetime);

        Assert.assertTrue("获取的最大ID不正确！", text.getID() == (maxID - 1));
    }
    ...
    public void test_b_DeleteAllSync() {
        rtcp.deleteNObjectSync(BaseSQLiteBO.INVALID_CASE_ID, null);
        List<RetailTradeCommodity> tradeCommodityList = (List<RetailTradeCommodity>) rtcp.retrieveNObjectSync(BaseSQLiteBO.INVALID_CASE_ID, null);
        Assert.assertTrue("RetrieveNSync测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        Assert.assertTrue("RetrieveNSync搜索到的数据数量应该=0!", tradeCommodityList.size() == 0);
    }
    ...
    public void test_c_CreateNSync() throws Exception {
        ...
        rtcp.createNObjectSync(BaseSQLiteBO.INVALID_CASE_ID, commodityList);

        Assert.assertTrue("CreateNSync1测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        //
        for (int i = 0; i < commodityList.size(); i++) {
            RetailTradeCommodity retailTradeCommodity = (RetailTradeCommodity) rtcp.retrieve1ObjectSync(BaseSQLiteBO.INVALID_CASE_ID, commodityList.get(i));
            Assert.assertTrue("CreateNSync测试失败,原因retrieve1ObjectSync时返回的错误码不正确！", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
            Assert.assertTrue("CreateNSync测试失败,原因:所插入数据与查询到的不一致!", commodityList.get(i).compareTo(retailTradeCommodity) == 0);
        }
    }
    ...
    public void test_d_CreateSync() throws CloneNotSupportedException {
        ...
        rtcp.createObjectSync(BaseSQLiteBO.INVALID_CASE_ID, retailTradeCommodity);

        Assert.assertTrue("CreateSync bm1测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        //
        RetailTradeCommodity comm1Retrieve1 = (RetailTradeCommodity) rtcp.retrieve1ObjectSync(BaseSQLiteBO.INVALID_CASE_ID, retailTradeCommodity);
        //
        Assert.assertTrue("retrieve1ObjectSync返回的错误码不正确！", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        Assert.assertTrue("CreateSync测试失败,原因:所插入数据与查询到的不一致!", retailTradeCommodity.compareTo(comm1Retrieve1) == 0);
        ...
        rtcp.createObjectSync(BaseSQLiteBO.INVALID_CASE_ID, retailTradeCommodity1);

        Assert.assertTrue("CreateSync bm1重复ID测试失败,原因:返回错误码不应该为EC_NoError!", rtcp.getLastErrorCode() != ErrorInfo.EnumErrorCode.EC_NoError);
    ...
    public void test_e_RetrieveNSync() throws CloneNotSupportedException {
        ...
        List<RetailTradeCommodity> RetailTradeCommodityList = (List<RetailTradeCommodity>) rtcp.retrieveNObjectSync(BaseSQLiteBO.INVALID_CASE_ID, null);
        Assert.assertTrue("RetrieveNSync测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        //
        Assert.assertTrue("RetrieveNSync搜索到的数据数量应该>=0!", RetailTradeCommodityList.size() >= 0);

        //根据条件查询 //...将来增加case：输入更多的查询条件
        RetailTradeCommodity retailTradeCommodity = new RetailTradeCommodity();
        retailTradeCommodity.setSql("where F_TradeID = ?");
        retailTradeCommodity.setConditions(new String[]{"1"});
        RetailTradeCommodityList = (List<RetailTradeCommodity>) rtcp.retrieveNObjectSync(BaseSQLiteBO.CASE_RetailTradeCommodity_RetrieveNByConditions, retailTradeCommodity);
        Assert.assertTrue("根据条件 RetrieveNSync 测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        //
        Assert.assertTrue("根据条件RetrieveNSync搜索到的数据数量应该>=0!", RetailTradeCommodityList.size() >= 0);

        //异常的case,一个通配符多个值
        RetailTradeCommodity retailTradeCommodity1 = new RetailTradeCommodity();
        retailTradeCommodity1.setSql("where F_TradeID=?");
        retailTradeCommodity1.setConditions(new String[]{"1","2"});
        rtcp.retrieveNObjectSync(BaseSQLiteBO.CASE_RetailTradeCommodity_RetrieveNByConditions, retailTradeCommodity1);
        Assert.assertTrue("根据条件 RetrieveNSync 测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() != ErrorInfo.EnumErrorCode.EC_NoError);

        //异常的case,多个通配符一个值
        RetailTradeCommodity retailTradeCommodity2 = new RetailTradeCommodity();
        retailTradeCommodity2.setSql("where F_ID=? and F_TradeID=?");
        retailTradeCommodity2.setConditions(new String[]{"1"});
        rtcp.retrieveNObjectSync(BaseSQLiteBO.CASE_RetailTradeCommodity_RetrieveNByConditions, retailTradeCommodity2);
        Assert.assertTrue("根据条件 RetrieveNSync 测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);

        //异常的case,多个通配符多个值
        RetailTradeCommodity retailTradeCommodity3 = new RetailTradeCommodity();
        retailTradeCommodity3.setSql("where F_ID=? and F_TradeID=?");
        retailTradeCommodity3.setConditions(new String[]{"1", "1"});
        rtcp.retrieveNObjectSync(BaseSQLiteBO.CASE_RetailTradeCommodity_RetrieveNByConditions, retailTradeCommodity3);
        Assert.assertTrue("根据条件 RetrieveNSync 测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
    ...
    public void test_f_Retrieve1Sync() throws CloneNotSupportedException {
        ...
        //正常的case
        Assert.assertTrue("CreateSync bm1测试失败,原因:返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        RetailTradeCommodity retailTradeCommodity1 = (RetailTradeCommodity) rtcp.retrieve1ObjectSync(BaseSQLiteBO.INVALID_CASE_ID, retailTradeCommodity);
        Assert.assertTrue("Retrieve1 测试失败,原因返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        Assert.assertTrue("Retrieve1 测试失败,查询的数据不正确!", retailTradeCommodity1 != null);
        ...
        RetailTradeCommodity retailTradeCommodity3 = (RetailTradeCommodity) rtcp.retrieve1ObjectSync(BaseSQLiteBO.INVALID_CASE_ID, retailTradeCommodity2);
        Assert.assertTrue("Retrieve1 测试失败,原因返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        Assert.assertTrue("Retrieve1 测试失败,返回的basemodel不为null", retailTradeCommodity3 == null);
    }
    ...
    public void test_g_DeleteSync() throws CloneNotSupportedException {
        ...
        //正常的case
        RetailTradeCommodity retailTradeCommodity2 = DataInput.getRetailTradeCommodity();
        retailTradeCommodity2.setID(retailTradeCommodity.getID());
        rtcp.deleteObjectSync(BaseSQLiteBO.INVALID_CASE_ID, retailTradeCommodity2);
        Assert.assertTrue("DeleteSync 测试失败,原因返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        RetailTradeCommodity retailTradeCommodity3 = (RetailTradeCommodity) rtcp.retrieve1ObjectSync(BaseSQLiteBO.INVALID_CASE_ID, retailTradeCommodity2);
        Assert.assertTrue("Retrieve1 测试失败,原因返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
        Assert.assertTrue("Retrieve1 测试失败,查询的数据不正确!", retailTradeCommodity3 == null);
        ...
        rtcp.deleteObjectSync(BaseSQLiteBO.INVALID_CASE_ID, retailTradeCommodity4);
        Assert.assertTrue("DeleteSync 测试失败,原因返回错误码不正确!", rtcp.getLastErrorCode() == ErrorInfo.EnumErrorCode.EC_NoError);
