...
public class DialogCommodityRecyclerViewAdapter extends RecyclerView.Adapter<DialogCommodityRecyclerViewAdapter.MyViewHolder> {
    List<Commodity> list;//存放数据
    Context context;
    String code;
    int pos;
    private int defItem = -1;
    private DialogCommodityRecyclerViewAdapter.OnItemListener onItemListener;

    public int isShowInventory = 0;

    public DialogCommodityRecyclerViewAdapter(List<Commodity> list, Context context) {
        this.list = list;
        this.context = context;
    }

    public void setOnItemListener(DialogCommodityRecyclerViewAdapter.OnItemListener onItemListener) {
        this.onItemListener = onItemListener;
    }

    public interface OnItemListener {
        void onClick(DialogCommodityRecyclerViewAdapter.MyViewHolder holder, int position);
    }
    ...
    public MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        MyViewHolder holder = new MyViewHolder(LayoutInflater.from(context).inflate(R.layout.alertdialog_commodity_rv_item, parent, false));
        return holder;
    }
...
// 在这里对获取对象进行操作
    ...
    public void onBindViewHolder(@NonNull final MyViewHolder holder, final int position) {
        ...
        holder.retail_price.setText(GeneralUtil.formatToShow(list.get(position).getPriceRetail()));
        holder.inventory.setText(String.valueOf(list.get(position).getNO()));
        if (position % 2 == 0) {
