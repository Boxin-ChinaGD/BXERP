DROP PROCEDURE IF EXISTS `SP_Commodity_CreateMultiPackaging`;
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_Commodity_CreateMultiPackaging`(
	OUT iErrorCode INT,
...
BEGIN
    ...
    START TRANSACTION;
		...
		ELSEIF NOT EXISTS (SELECT 1 FROM t_packageunit WHERE F_ID = iPackageUnitID) THEN
	   	...
	   	ELSEIF NOT EXISTS (SELECT 1 FROM t_commodity WHERE F_ID = sRefCommodityID AND F_Type = 0) THEN
			SET iErrorCode := 7;
			SET sErrorMsg := '多包装商品的参照单品不存在';
