SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case31.sql+++++++++++++++++++++++';

SELECT '----- case31:员工A创建一个零售单卖A商品，对应两个入库单 ,卖B商品，对应两个入库单 , 卖C商品，对应两个入库单,员工B隔天创建一张退货单，对商品A、B全部退货[A、B]-------' AS 'Case31';
-- 
-- 员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
-- 插入入库单
SET @warehouseA1NO = 219;
SET @warehouseA2NO = 299;
...
INSERT INTO t_commodity (F_Status, F_Name, F_ShortName, F_Specification, F_PackageUnitID, F_PurchasingUnit, F_BrandID, F_CategoryID, F_MnemonicCode, F_PricingType, 
F_PriceVIP, F_PriceWholesale, F_CanChangePrice, F_RuleOfPoint, F_ShelfLife, F_ReturnDays, F_CreateDate, F_PurchaseFlag, F_RefCommodityID, F_RefCommodityMultiple, F_Tag, F_Type, F_CreateDatetime, F_UpdateDatetime)
VALUE (0, '跨库商品C', '普通商品C', '个', 3, '包', 1, 1, 'CJS666', 1, 
19, 19, 0, 1, 365, 3, '2119-01-01 09:00:00', 1, 0, 0, 0, 0, '2119-01-01 09:00:00', '2019-05-19 09:00:00');
...
-- 员工B新建退货单
SET @returnCommANO_B = @warehouseA1NO + @warehouseA2NO;
SET @returnCommBNO_B = @warehouseB1NO + @warehouseB2NO;
...
SET @rrtTotalAmount_B = @returnCommANO_B * @aCommotidyPrice + @returnCommBNO_B * @bCommotidyPrice;
...
-- 插入零售单商品来源
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rrtCommAID_B, @commIDA, @warehouseA1NO, @warehouseA1);
SET @returnCommsourceIDA1_B = last_insert_id();
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rrtCommAID_B, @commIDA, @warehouseA2NO, @warehouseA2);
SET @returnCommsourceIDA2_B = last_insert_id();
-- 零售商品B
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
...
-- 插入零售单商品来源
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rrtCommBID_B, @commIDB, 50, @warehouseB1);
SET @returnCommsourceIDB1_B = last_insert_id();
INSERT INTO t_retailtradecommoditysource (F_RetailTradeCommodityID, F_ReducingCommodityID, F_NO, F_WarehousingID)
VALUES (@rrtCommBID_B, @commIDB, 50, @warehouseB2);
SET @returnCommsourceIDB2_B = last_insert_id();

-- 
-- 插入退货去向表
SET @NOReturnToA1 = @warehouseA1NO; -- 全退
SET @NOReturnToA2 = @warehouseA2NO;	-- 全退
SET @NOReturnToB1 = @warehouseB1NO;	-- 全退
SET @NOReturnToB2 = @warehouseB2NO;	-- 全退
...
SET @rggWorkTimeEnd_A='2041/02/01 12:00:00';
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case31 Result';
...
AND F_Datetime = @dSaleDatetime
AND F_TotalAmount = @totalAmount_B -- AND @totalAmount_B < 0
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case31 Result';
...
DELETE FROM t_retailtradecommoditysource WHERE F_ID = @returnCommsourceIDA1_B;
DELETE FROM t_retailtradecommoditysource WHERE F_ID = @returnCommsourceIDA2_B;
DELETE FROM t_retailtradecommoditysource WHERE F_ID = @returnCommsourceIDB1_B;
DELETE FROM t_retailtradecommoditysource WHERE F_ID = @returnCommsourceIDB2_B;
