SELECT '----- case32:员工A创建一个零售单卖A商品，对应两个入库单 ,卖B商品，对应两个入库单 , 卖C商品，对应两个入库单,员工B隔天创建一张退货单，对商品A、B部分退货[A、B]-------' AS 'Case32';
-- 
--  员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
-- 
-- 员工B新建退货单
SET @returnCommANO_B = 250;
SET @returnCommBNO_B = 50;
...
-- 
-- 插入退货去向表
SET @NOReturnToA1 = 100;
SET @NOReturnToA2 = 150;
SET @NOReturnToB1 = 25;
SET @NOReturnToB2 = 25;
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case32 Result';
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case32 Result';
-- 
