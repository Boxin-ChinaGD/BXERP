SELECT '----- case35:员工A创建一个零售单卖A商品，对应两个入库单 ,卖B商品，对应两个入库单 , 卖C商品，对应两个入库单,员工B创建一张退货单，对商品A、B全部退货、对C部分退货[A、B]-------' AS 'Case35';
-- 
-- 员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case35 Result';
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case35 Result';
-- 
