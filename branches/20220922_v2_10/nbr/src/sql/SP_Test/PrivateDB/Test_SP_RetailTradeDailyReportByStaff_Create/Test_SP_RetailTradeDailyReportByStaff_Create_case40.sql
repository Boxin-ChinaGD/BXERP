SELECT '----- case40:员工A创建一个零售单卖A商品，对应两个入库单 ,卖B商品，对应两个入库单 , 卖C商品，对应两个入库单,员工B创建一张退货单，对商品A、B、C全部退货[AB]-------' AS 'Case40';
-- 
-- 员工ID
SET @staffIDA = 9;
SET @staffIDB = 10;
...
SET @syncDatetime_A = '2041/02/02 06:00:00';
...
VALUES (@staffIDA, 2, @rggWorkTimeStart, @rggWorkTimeEnd, @rggNO, @rggTotalAmount_A, 0.000000, @rggTotalAmountCash_A, @rggTotalAmountWeChat_A, @rggTotalAmountAliPay_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
SET @rtgID_A = last_insert_id();
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDB, 2, @rggWorkTimeStart, @rggWorkTimeEnd, @rggNO, @rggTotalAmount_B, 0.000000, @rggTotalAmountCash_B, @rggTotalAmountWeChat_B, @rggTotalAmountAliPay_B, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case40 Result';
-- 
-- 结果验证
-- 验证员工B在02/02当天没有业绩(退了A的单，但A有上班)
-- 售卖总金额
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case40 Result';
-- 
