SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case44.sql+++++++++++++++++++++++';
SELECT '----- case44:员工A创建一个零售单卖A商品，对应两个入库单 ,B商品，对应两个入库单 , C商品，对应两个入库单,员工B隔天创建一张退货单，对商品A、B、C全部退货[A、B]-------' AS 'Case44';
...
-- 插入入库单商品表
SET @warehouseA1NO = 471;
SET @warehouseA2NO = 139;
SET @warehouseA1Price = 13.31;
SET @warehouseA2Price = 15.21;
...
-- 插入入库单商品表
SET @warehouseB1NO = 550;
SET @warehouseB2NO = 432;
SET @warehouseB1Price = 17.760000;
SET @warehouseB2Price = 12.350000;
...
-- 插入入库单商品表
SET @warehouseC1NO = 311;
SET @warehouseC2NO = 411;
...
-- 零售商品B
INSERT INTO t_retailtradecommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer, F_PriceVIPOriginal)
VALUES (@returnID_B, @commIDB, '可口可乐', @barcoMdeIDB, @returnCommBNO_B, @bCommotidyPrice, @returnCommBNO_B, @bCommotidyPrice, 200, NULL);
...
VALUES (@staffIDA, 2, @rggWorkTimeStart1, @rggWorkTimeEnd1, @rggNO_A, @rggTotalAmount_A1, 0.000000, @rggTotalAmountCash_A1, @rggTotalAmountWeChat_A1, @rggTotalAmountAliPay_A1, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case44 Result';
...
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDB, 2, @rggWorkTimeStart2, @rggWorkTimeEnd2, @rggNO_B, @rggTotalAmount_B, 4000.000000, @rggTotalAmountCash_B, @rggTotalAmountWeChat_B, @rggTotalAmountAliPay_B, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
-- 验证员工B在02/02虽然有退员工A的单，但是员工A不在上班，算员工B的业绩
-- 售卖总金额
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_B = 1,'测试成功','测试失败') AS 'Test Case44 Result';
-- 
