SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case47.sql+++++++++++++++++++++++';


SELECT '----- case47:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,创建一张退货单，对商品A全部退货,对B部分退货,昨天买，今天退 [A、A]-------' AS 'Case47';
...
-- 插入入库单商品表
SET @warehouseA1NO = 510;
SET @warehouseA2NO = 500;
...
-- 插入入库单商品表
SET @warehouseB1NO = 310;
SET @warehouseB1Price = 17.230000;
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
...
-- 插入入库单商品表
SET @warehouseC1NO = 201;
SET @warehouseC1Price = 12.360000;
INSERT INTO t_warehousingcommodity (F_WarehousingID, F_CommodityID, F_NO, F_PackageUnitID, F_CommodityName, F_BarcodeID, F_Price, F_Amount, F_ProductionDatetime, F_ShelfLife, F_ExpireDatetime, F_CreateDatetime, F_UpdateDatetime, F_NOSalable)
...
SET @saleDatetimeYesterday_A = '2041/02/01 06:00:00';
SET @syncDatetimeYesterday_A = '2041/02/01 06:00:00';
...
VALUES (1,'LS2041020200000500010001',11068,1,'url=ashasoadigmnalskd',@saleDatetimeYesterday_A,@staffIDA,1,'0',1,'…',-1,@syncDatetimeYesterday_A, @rtTotalAmount_A, @rtAmuontCash_A, @rtAmuontAliPay_A,@rtAmuontWeChat_A,0.000000,0.000000,0.000000,0.000000,0.000000,1,NULL,NULL,NULL,NULL,NULL,NULL,2);
...
SET @returnCommBNO_A = 222; -- 部分退,总300
...
VALUES (@staffIDA, 2, @rggWorkTimeStart1, @rggWorkTimeEnd1, @rggNO1_A, @rggTotalAmount_A1, 1600.000000, @rggTotalAmountCash_A1, @rggTotalAmountWeChat_A1, @rggTotalAmountAliPay_A1, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification_A = 1,'测试成功','测试失败') AS 'Test Case47 Result';

-- 零售单收银汇总
-- 用于02/02员工A的业绩验证,A只退货，不售卖
SET @rggWorkTimeStart2='2041/02/02 00:00:00';
SET @rggWorkTimeEnd2='2041/02/02 23:59:59';
SET @rggTotalAmount2_A = 0.000000;
SET @rggTotalAmountCash2_A = 0.000000;
SET @rggTotalAmountWeChat2_A = 0.000000;
SET @rggTotalAmountAliPay2_A = 0.000000;
SET @rggNO2_A = 0;
INSERT INTO t_retailtradeaggregation (F_StaffID, F_PosID, F_WorkTimeStart, F_WorkTimeEnd, F_TradeNO, F_Amount, F_ReserveAmount, F_CashAmount, F_WechatAmount, F_AlipayAmount, F_Amount1, F_Amount2, F_Amount3, F_Amount4, F_Amount5, F_UploadDateTime)
VALUES (@staffIDA, 2, @rggWorkTimeStart2, @rggWorkTimeEnd2, @rggNO2_A, @rggTotalAmount2_A, 160.000000, @rggTotalAmountCash2_A, @rggTotalAmountWeChat2_A, @rggTotalAmountAliPay2_A, 0.000000, 0.000000, 0.000000, 0.000000, 0.000000, now());
...
-- 验证员工A在02/02只有退员工A的单
-- 售卖总金额
SET @saleTotalAmount2_A = 0.000000;
-- 售卖商品总成本
SET @saleTotalCost2_A = 0.000000;
-- 售卖商品总毛利
SET @saleTotalMargin2_A = @saleTotalAmount2_A - @saleTotalCost2_A;
-- 退货总金额
SET @returnTotalAmount2_A = @rrtTotalAmount_A;
-- 退货商品总成本
SET @returnTotalCost2_A = (@NOReturnToA1 * @warehouseA1Price + @NOReturnToA2 * @warehouseA2Price) + 
						 (@NOReturnToB1 * @warehouseB1Price);

-- 退货商品总毛利
SET @returnTotalMargin2_A = @returnTotalAmount2_A - @returnTotalCost2_A;
-- 总金额（售卖总金额 - 退货总金额）
SET @totalAmount2_A = @saleTotalAmount2_A - @returnTotalAmount2_A;
-- 总毛利（售卖商品总毛利 - 退货商品总毛利）
SET @grossMargin2_A = @saleTotalMargin2_A - @returnTotalMargin2_A;

SET @ResultVerification2_A=0;
-- 进行员工A业绩报表的数据结果验证
SELECT 1 INTO @ResultVerification2_A
FROM t_retailtradedailyreportbystaff 
WHERE F_StaffID = @staffIDA 
AND F_Datetime = @dSaleDatetime
AND F_TotalAmount = @totalAmount2_A
AND F_GrossMargin = @grossMargin2_A
AND F_NO = (
	SELECT COUNT(1) 
	FROM t_retailtrade 
	WHERE F_SourceID = -1 
...
SELECT IF(@iErrorCode = 0 AND @ResultVerification2_A = 1,'测试成功','测试失败') AS 'Test Case47 Result';
-- 
DELETE FROM t_retailtradedailyreportbystaff WHERE F_Datetime = @dSaleDatetimeYesterday;
