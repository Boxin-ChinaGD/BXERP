SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case58.sql+++++++++++++++++++++++';

SELECT '----- case58:员工A创建一个零售单卖A商品，对应两个入库单(一次入库价位15，一次为10) ,B商品，对应一个入库单 , C商品，对应一个入库单,员工B创建一张退货单，对商品A、B、C全部退货,上午卖，下午退 [A、B]-------' AS 'Case58';
-- 收银员
SET @staffID_A = 9;
SET @staffID_B = 10;
...
SET @warehousingCommodityA_Price1 = 11.1; -- 入库单1
SET @warehousingCommodityA_NO1 = 500;
SET @warehousingCommodityA_Price2 = 22.2; -- 入库单2
SET @warehousingCommodityA_NO2 = 500;
SET @commodityPriceB = 220;
SET @warehousingCommodityB_Price = 33.3;
SET @warehousingCommodityB_NO = 500;
SET @commodityPriceC = 770;
SET @warehousingCommodityC_Price = 44.4;
...
SET @rt1CommotidyPrice_A = 111;
SET @rt1CommotidyNO_B = 333;
SET @rt1CommotidyPrice_B = 222;
SET @rt1CommotidyNO_C = 444;
SET @rt1CommotidyPrice_C = 333;
...
SET @rggTotalAmountCash_B = @rrt1AmuontCash_B;
SET @rggTotalAmountWeChat_B = @rrt1AmuontWeChat_B;
SET @rggTotalAmountAliPay_B = @rrt1AmuontAliPay_B;
SET @rggNO_B = 0;
...
SELECT IF(@iErrorCode = 0, '测试成功', '测试失败') AS 'Case58.1 Testing Result';
-- 
...
SELECT IF(@countDefaultData = 1, '测试成功', '测试失败') AS 'Case58.2 Testing Result';
SET @totalAmountA = 0;
...
SELECT IF(@totalAmountA = @rt1TotalAmount_A, '测试成功', '测试失败') AS 'Case58.3 Testing Result';
-- 
...
SELECT IF(@retailtradeNOA = @rtNoByStaffA, '测试成功', '测试失败') AS 'Case58.4 Testing Result';
-- 
-- 进行业绩报表的毛利结果验证
...
SELECT IF(@grossMargin = @rt1TotalAmount_A - (@rt1CommotidyNO1_A * @warehousingCommodityA_Price1) - (@rt1CommotidyNO2_A * @warehousingCommodityA_Price2) -- 
							- (@rt1CommotidyNO_B * @warehousingCommodityB_Price) - (@rt1CommotidyNO_C * @warehousingCommodityC_Price), '测试成功', '测试失败') AS 'Case58.5 Testing Result';
-- 
...
SELECT IF(@countDefaultDataB = 1, '测试成功', '测试失败') AS 'Case58.6 Testing Result';
SET @totalAmountB = 0;
...
SELECT IF(@totalAmountB = 0 - @rrt1TotalAmount_B, '测试成功', '测试失败') AS 'Case58.7 Testing Result';
...
SELECT IF(@retailtradeNOB = @rtNoByStaffB, '测试成功', '测试失败') AS 'Case58.8 Testing Result';
...
SELECT IF(@grossMarginB = (@rt1CommotidyNO1_A * @warehousingCommodityA_Price1) + (@rt1CommotidyNO2_A * @warehousingCommodityA_Price2) -- 
							+ (@rt1CommotidyNO_B * @warehousingCommodityB_Price) + (@rt1CommotidyNO_C * @warehousingCommodityC_Price) - @rrt1TotalAmount_B, '测试成功', '测试失败') AS 'Case58.9 Testing Result';
-- 删除测试创建的数据
