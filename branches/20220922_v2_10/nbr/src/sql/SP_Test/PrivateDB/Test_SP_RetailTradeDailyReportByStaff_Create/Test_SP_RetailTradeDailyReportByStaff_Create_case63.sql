SELECT '++++++++++++++++++Test_SP_RetailTradeDailyReportByStaff_Create_case63.sql+++++++++++++++++++++++';

SELECT '----- case63:  员工A创建一个零售单卖A商品，商品A为未入库商品;卖B商品，对应一个入库单 [A] -------' AS 'Case63';
...
INSERT INTO T_RetailTradeCommodity (F_TradeID, F_CommodityID, F_CommodityName, F_BarcodeID, F_NO, F_PriceOriginal, F_NOCanReturn, F_PriceReturn, F_PriceSpecialOffer)
VALUES (@rtID1, @commIDB,'未入库商品B', @barcodeIDB, @rt1CommotidyNO2_B, @rt1CommotidyPrice2_B, @rt1CommotidyNO2_B, @rt1CommotidyPrice2_B, @rt1CommotidyPrice2_B);
...
DELETE FROM t_providercommodity WHERE F_CommodityID IN (@commIDB,@commIDA) ;
DELETE FROM t_barcodes WHERE F_CommodityID IN (@commIDB,@commIDA) ;
DELETE FROM t_commodity WHERE F_ID IN (@commIDB,@commIDA) ;