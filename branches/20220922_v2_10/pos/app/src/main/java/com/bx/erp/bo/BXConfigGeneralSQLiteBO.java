...
public class BXConfigGeneralSQLiteBO extends BaseSQLiteBO  {
    ...
    public BXConfigGeneralSQLiteBO(Context ctx, BaseSQLiteEvent sEvent, BaseHttpEvent hEvent) {
        context = ctx;
        sqLiteEvent = sEvent;
        httpEvent = hEvent;
    }
    ...
    protected boolean applyServerListDataAsyncC(List<BaseModel> bmList) {
        ...
        return GlobalController.getInstance().getBXConfigGeneralPresenter().refreshByServerDataObjectsAsyncC(BaseSQLiteBO.INVALID_CASE_ID, bmList, sqLiteEvent);
    }
