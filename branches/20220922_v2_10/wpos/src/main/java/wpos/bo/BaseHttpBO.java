...
//    }
    ...
    public boolean createAsync(int iUseCaseID, BaseModel bm) {
        ...
        // 判断SessionID是否为null，如果为null，返回false
        if (GlobalController.getInstance().getSessionID() == null || (!NetworkUtils.isNetworkAvalible())) {
            log.info("GlobalController.getInstance().getSessionID():" + GlobalController.getInstance().getSessionID() + "\t" + "NetworkUtils.isNetworkAvalible():" + NetworkUtils.isNetworkAvalible());
            GlobalController.getInstance().setSessionID(null);
            httpEvent.setLastErrorCode(ErrorInfo.EnumErrorCode.EC_InvalidSession);
            return false;
        }
