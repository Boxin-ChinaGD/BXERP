...
public abstract class BasePromotion {
     ...
     */
    protected Map<Integer, List<RetailTradeCommodity>> mapCommodity = new HashMap<Integer, List<RetailTradeCommodity>>();
...
//    }
    ...
    // 保存每个促销活动 的详细计算过程
    protected Map<Integer, RetailTradePromotingFlow> mapRetailTradePromotingFlow = new HashMap<Integer, RetailTradePromotingFlow>();
    ...
    public void prePromote() {
        ...
        for (RetailTradeCommodity rtc : commodityList) {
            //ErrorCode ecOut = new ErrorCode(); POS没有缓存
            // Commodity commodity = (Commodity) CacheManager.getCache(EnumCacheType.ECT_Commodity).read1(rtc.getCommodityID(), BaseBO.SYSTEM, ecOut);
//            if (commodity == null) {
//                // ...
...
//            rtc.setPriceReturn(commodity.getPriceRetail());
    ...
    public void promote(RetailTrade rt, EnumPromotionScope eps, final List<BaseModel> listPromotion) {
        ...
        for (Object o : listPromotion) {
            ...
            if (isTargetPromotion(promotion) && DatetimeUtil.isInTimeRange(promotion.getDatetimeStart(), nowData, promotion.getDatetimeEnd())) {
        ...
        if (!promoted) {
            log.info("范围为" + eps.toString() + "时没有找到有" + getPromotionType() + "的促销活动！");
        } else {
            // 有参与过活动mn
    ...
    public void calculateReturnPrice(RetailTrade rt, double fBestAmount, EnumPromotionScope eps) {
        ...
        for (RetailTradeCommodity rtc : retailTradeCommodityList) {
            // 原来的计算过程是 rtc.getPriceReturn() * rtc.getNO() / getOriginalTotalAmount() * fBestAmount / rtc.getNO()
            // 现改为 (rtc.getPriceReturn() * rtc.getNO() * fBestAmount) / (getOriginalTotalAmount() * rtc.getNO())
            // 原因是，按照先乘后除的计算顺序可以减少计算误差
            rtc.setPriceReturn(Double.valueOf(GeneralUtil.formatToCalculate(GeneralUtil.div(GeneralUtil.mul(GeneralUtil.mul(rtc.getPriceReturn(), rtc.getNO()), fBestAmount), GeneralUtil.mul(getOriginalTotalAmount(), rtc.getNO()), 6))));
        }
    }
